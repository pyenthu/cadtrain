...
export const KERNEL_VERSION = 'manifold-3d@3.4.1+n3scale'; // +cap1: ear-clip r_sweep end caps (fanCap3D, manifold-mesh) — a transitive engine import NOT in scriptHash, so its fix needs a manual kernel bump to bust the stale-cap IndexedDB cache. +nrm4: crease-aware render normals under vert ceiling. +cut2: sectionCut refines the CUT RESULT, not just the wedge (#64 bridging triangle) — same transitive-import problem, same manual bump. +n3scale: warp-spline.ts gained build-time xDiaScale/yScale (N3) + survey-to-xyz.ts joined the sandbox (N3b) — both transitive engine imports, so bump to bust the stale-mesh IndexedDB cache

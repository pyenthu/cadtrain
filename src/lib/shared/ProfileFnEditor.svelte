<script lang="ts">
  ...
  // Build = calculated EXPRESSIONS + a visual MOVE list (the pen path). The
  // pure round-trip (`Move`/`Calc` types, `splitArgs`, `parseBody`,
  // `composeBody`) lives in `./profile-fn-compose` (modularize R9).
  ...
  // Pure string-building lives in `composeBody` (./profile-fn-compose); this
  // thin wrapper feeds it the reactive state (rows → keys, calc, moves, seed)
  // and adds the `export function build(p) { … }` wrapper.
  function composeSource(): string {
    return `export function build(p) {\n${composeBody({ keys: rows.map((r) => r.key), calc, moves, seedBody: seed?.body })}\n}`;
  }

...
export function cardMinWidth(node: any): number {
  ...
  if (node.type === 'spline') return 84;   // tiny: curve glyph + buttons + socket
  return 130;
}
...
export function cardAutoWidth(graph: Graph, node: any): number {
  ...
  if (node.type === 'spline') return 96;   // just the curve glyph + buttons + socket
...
export function nodeSize(graph: Graph, node: any): { w: number; h: number } {
  ...
  if (node.type === 'spline') return { w, h: 44 };
  return { w, h: 80 };
}

...
function systemPrompt(app: AppDoc, grounding = ''): string {
  ...
  return [
    ...
    'A panel has a "kind" (list, form, table, grid, bake3d, svg, text, chat, container, card, button, tabs, toolbar) and may carry a "source" binding {verb, args} plus "controls". grid = a read-only data table from any source (props.columns); tabs/toolbar = layout containers with children (tabs: props.labels; toolbar: props.align).',

...
export function systemPrompt(app: AppDoc, grounding = '', prompt = ''): string {
  ...
  return [
    ...
    'Data structures: app.structures { name: [{name,type},…] } — a reusable RECORD SCHEMA (field list). "Define a <name> data structure with fields a, b, c" → patchApp {op:"set", path:"structures.<name>", value:[{name:"a",type:"string"},{name:"b",type:"number"},…]}. A table/grid adopts one via props.structure.',

...
export function emitInstruction(): string {
  ...
  return [
    ...
    'describes — do NOT default everything to text/list.',
    'INCREMENTAL: this is ONE instruction in a longer sequence — the "Current app" above is what',
    'already exists. Emit verbs for ONLY what THIS instruction asks, and do NOT re-create or duplicate',
    'anything already in the Current app. Do NOT pre-build panels, variables, or theme the instruction',
    "doesn't mention — later instructions add those. If it only sets the title, emit ONLY setAppMeta;",
    'if it only adds one heading, emit ONLY that one definePanel. Usually that is one or two verbs.',
    'Before adding a panel, check the Current app\'s panels: if one of that kind/purpose already exists,',
    'do NOT add another (no duplicate headings/gantts/tables).',
    '',
    'Below is the verb SHAPE for each kind of instruction. Words in ALL-CAPS (TITLE, VAR, LABEL, COLS,',
    'HEX) are PLACEHOLDERS — replace each with the value from THIS instruction. NEVER emit these literal',
    'example words; they show format only, not content. One instruction → its verb(s):',
    '  "make it a roadmap app titled TITLE"      -> [{"verb":"setAppMeta","args":{"title":"TITLE","docType":"roadmap"}}]',
    '  "seed a VAR variable with these rows"      -> [{"verb":"patchApp","args":{"op":"set","path":"vars.VAR","value":[ ...the rows the instruction lists... ]}}]',
    '  "define a VAR structure with fields a,b"   -> [{"verb":"patchApp","args":{"op":"set","path":"structures.VAR","value":[{"name":"a","type":"string"},{"name":"b","type":"number"}]}}]',
    '  "add a heading LABEL"                       -> [{"verb":"definePanel","args":{"panel":{"id":"hd","kind":"heading","props":{"text":"LABEL","level":1}}}}]',
    '  "show VAR as a table"                       -> [{"verb":"definePanel","args":{"panel":{"id":"tb","kind":"grid","source":{"verb":"readVar","args":{"name":"VAR"}},"props":{"columns":"COLS"}}}}]',
    '  "use a light theme with a HEX accent"       -> [{"verb":"patchApp","args":{"op":"set","path":"theme","value":{"mode":"light","accent":"HEX"}}}]',
  ].join('\n');
}

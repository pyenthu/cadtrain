<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 624, bundle: 'K', lane: 10, start: 29.8, weeks: 1.0, priority: 'high',   status: 'open',   title: 'K.24 — Profiles first-class A: a Profiles section in the /primitives sidebar listing every profile function (curated + ƒ volume), and a profile TAB type (click → ProfileFnEditor full-screen, save like a primitive). See docs/plans/profiles-first-class.md.' },
    { id: 625, bundle: 'K', lane: 10, start: 30.8, weeks: 1.2, priority: 'high',   status: 'open',   title: 'K.25 — Profiles first-class B: profile = a part PARAMETER (pick + lift). New `profile` param type (ProfilePalette selector); swap re-lifts the profile\'s params; "open in tab ↗" button; P6 bake resolves a profile whose kind comes from a param value (not just a literal). docs/plans/profiles-first-class.md.' },
    { id: 626, bundle: 'K', lane: 10, start: 32.0, weeks: 0.8, priority: 'high',   status: 'open',   title: 'K.26 — Profiles first-class C: kill vertex profiles. Load… r_revolve/r_extrude scaffold function-first (never a vertex array); retire the draggable vertex editor from the profile flow (leafEdit / profileEdit literal+profile modes / ProfileEditor — warp-path keeps it). docs/plans/profiles-first-class.md.' },
    { id: 627, bundle: 'K', lane: 10, start: 32.8, weeks: 0.4, priority: 'medium', status: 'todo',   title: 'K.27 — Profiles first-class D: migrate legacy vertex parts (dp_new revolve2, archived dp_*) to function profiles or remove. docs/plans/profiles-first-class.md.' },

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 913, bundle: 'O', lane: 14, start: 88.5, weeks: 1, priority: 'medium', status: 'open', title: "O.9 — TXFMN transform card: one TxfmnNode {child, rot[3], offset[3]} replacing composed Mv+Rot, re-expanded at emit to byte-identical mv(rot(child)); compact ROT/MV table with per-row f-expr + param socket; hydrate-time migration of legacy mv/rot. ✅ MODEL LAYER DONE 2026-06-16 (e267231): TxfmnNode type + emit (identity-elision) + setTxfmnAxis/addTxfmn mutators + one-way hydrate fold + 11 tests, 23/23 green. LEFT: the card render in GEP (the ROT/MV table UI). Plan: docs/plans/transform-card.md." },

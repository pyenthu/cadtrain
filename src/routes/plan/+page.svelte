<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 1020, bundle: 'F', lane: 5, start: 0.98, weeks: 0.18, priority: 'high', status: 'active', title: "#49 · Local-model replication — get a LOCAL model (Qwen3-4B in-browser / Ollama qwen3) to reproduce Claude's app builds (data-residency). SHIPPED: the eval harness (scripts/eval-app-build.ts + score-app.ts, promotion-invariant) · a parseVerbCalls small-model fallback (recovers lone {verb}/NDJSON) · the model survey (Qwen3-4B, thinking-mode OFF) · browser model swapped to Qwen3-4B. LOOP: Claude reference (plan .84 / design .93 / ewell .67) → local baseline → gap → RAG refine → repeat. First RAG targets from Rung 1: the STRUCTURES facet (plan+design at 0 — 'Define a X data structure' isn't emitting structures.X) + ewell's deep vtoolbar→iconbutton→popover nesting (~half reproduced). Ollama = headless proxy; Phi/Qwen3 = WebGPU-browser confirm. Plan: docs/plans/app-build-eval.md." },

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 943, bundle: 'B', lane: 1, start: 0.98, weeks: 0.10, priority: 'high', status: 'done', title: "g_dp_joint composite stack — mated children placed END-TO-END by Z-extent. DONE 2026-07-04. The TF/executor mated-stack builder now measures each built child's local Z-extent and shifts it onto a running down-Z cursor (placeEndToEnd + localZExtent in src/lib/shared/tf_examples/execute.ts, STACK_OVERLAP 0.1 for a clean weld) so a composite stack no longer piles at the origin — the g_dp_joint bottom collar is restored (box→tube→pin, total Z 37.8, was 25 with the pin buried). Stack-mode repeat now stacks copies end-to-end so g_dp_stand builds all N joints down +Z instead of overlapping. Fix commits: df45da1 (mated stack by Z-extent — g_dp_joint bottom collar restored), 1a0b4ef (stack-mode repeat stacks copies end-to-end — g_dp_stand no longer piles at origin), 978e696 (g_dp_stand builds all N joints stacked)." },
    { id: 970, bundle: 'B', lane: 1, start: 0.99, weeks: 0.30, priority: 'high', status: 'done', title: "Node-kind REGISTRY modularization (Phases 0-3) + GraphEditorController step-1 + undo/redo — SHIPPED 2026-07-07. Every graph node kind (18 descriptors) is now defined ONCE in src/lib/cad/nodes/ (node-kind.ts descriptor shape + registry.ts + kinds/*.ts), collapsing the ~525 LOC of duplicated emit/hydrate/layout switch statements into descriptor lookups, gated by a golden-emit byte-for-byte test (nodes/emit-golden.test.ts) so the refactor emits identical source. Phase 4 step-1 landed the GraphEditorController trunk (src/lib/shared/graph-editor/controller.svelte.ts — owns graph + history + leaf state) and undo/redo (graph-history.svelte.ts). The full Phase 4 controller extraction continues under B (active #940)." },
    { id: 971, bundle: 'B', lane: 1, start: 1.00, weeks: 0.25, priority: 'medium', status: 'done', title: "/design architecture visualization — SHIPPED 2026-07-07. Six architecture tabs (Tree · C4 model · Code graph [CodeGraph.svelte] · GEP module graph · UML class diagram · folder views) + SIX interchangeable folder layouts (FolderTree · FolderTreemap · FolderIcicle · FolderArc · FolderCirclePack · FolderEdgeBundle) over folder-hierarchy.ts. One-command snapshot regen via scripts/gen-design-diagrams.mjs (folder-tree + gep-modules), plus DEV-only buttons in /design that run the diagram-regen + graphify pipelines from the terminal at ZERO Claude tokens." },
    { id: 972, bundle: 'B', lane: 1, start: 1.01, weeks: 0.25, priority: 'high', status: 'done', title: "Parallel batch (5 worktree agents) — SHIPPED 2026-07-07. #37 transparency bleed fix (opaque-first renderOrder + depthTest on the per-part meshes so a transparent part no longer bleeds behind opaque siblings). #39 TF-warp regression (warp a composite/tube via revolve-tree DENSIFY instead of post-warp subdivide — kills the 53k-vert faceted blow-up, 53k→4.7k verts; wireframe toggle wired to the TF per-part / per-part-cut mesh path). #36c warp-trajectory PARTIAL — tangent extension of the spline to ±∞ shipped; the originZ placement option exists but is UNWIRED (tracked pending, #948). #38 data-driven params PARTIAL — P1 ParamSchema discriminated union (number|record|list) + hasKind + field emit + PT_LIST_GEOMETRY, and P3 parts_map producer (list<record> → N part instances, nodes/kinds/parts-map.ts) + the bakeable pm_demo example (bakes 3 solids); the GEP node-card UI + P2 param editor stay pending (#948). TF matte cut-interior section — the TF cut interior is now a matte grey material-group (was metallic)." },
    { id: 944, bundle: 'B', lane: 1, start: 1.02, weeks: 0.10, priority: 'medium', status: 'active', title: "Curvature-adaptive warp axial span — replace the constant _axialMaxZSpan (1.5) with a planAxialStations pass so long / gently-curved wells aren't over-tessellated (stations placed by local curvature, not a flat span). Follows Route C lean revolve (#942). Plan docs/plans/curvature-adaptive-warp-subdivision.md. PHASE 1 SHIPPED 2026-07-10: src/lib/wells/axial-density.ts derives ONE span for the whole well from the survey's MINIMUM radius of curvature, via the sagitta bound sag = R − √(R² − h²/4) ≈ h²/(8R) ⇒ h = √(8·R·tol), tol = 0.4% of the well's max radius, clamped to [0.5, 25] m; bake-worker-core gained `axialMaxZSpan?: number | null` (undefined ⇒ the old constant, byte-identical; null ⇒ no densification at all). 11 tests assert the ACTUAL sagitta of the chosen span, not the formula that produced it. TRAP: minCurvatureStep returns radCurvature = 999999, a SENTINEL not Infinity, so it sails straight through Number.isFinite — decide straightness on angPsi ≤ 1e-6 and compute R = dMD/angPsi yourself. STILL OPEN (this is the actual row): the span is GLOBAL — one number for the whole trajectory, set by the tightest dogleg — so a well with one hard kink still over-tessellates its straight sections. Phase 2 = per-segment stations. Also NOT YET VISIBLE: the only consumer is well-graph-bake.ts, and the /wells 3D tab that would mount it is still unwired (#42h), so this is a proven mechanism with no rendered effect." },
    { id: 945, bundle: 'B', lane: 1, start: 1.03, weeks: 0.12, priority: 'medium', status: 'open', title: "/api/tf/compile — parallelize the BFS dep frontier + save-invalidation. Fan the dep-resolution frontier out in parallel (today it walks serially), and INVALIDATE the dep-source caches on part Save so a cross-part edit is reflected instantly instead of waiting out the 30-60s TTL. Plan docs/plans/tf-compile-perf.md." },
    ...
    { id: 947, bundle: 'B', lane: 1, start: 1.04, weeks: 0.10, priority: 'medium', status: 'open', title: "Per-SUBPART material — a material set on a hidden sub-part currently can't be honored; needs color-by-source so a subpart's material survives into the merged full + cut meshes. Extends the per-PART material shipped with the TF hardening (#942)." },
    ...
    { id: 973, bundle: 'B', lane: 1, start: 1.015, weeks: 0.15, priority: 'high', status: 'done', title: "parts_map GEP node-card + pm_demo live volume part — SHIPPED 2026-07-07 (bdc2595). The data-driven-params (#38) authoring surface: NodeCard.svelte gained a VIOLET parts_map branch (src · rows · arg→field rows · list/stack/place op toggle · output socket · delete); composition-graph-mutate.ts gained addPartsMap + setPartsMapSrc/List/Arg/Op + add/removePartsMapArg; GraphEditorPane.svelte gained dropPartsMap + the ✎→container→parts_map picker item — so a list<record> → N-part producer is authored VISUALLY in the editor. PLUS pm_demo promoted to a LIVE VOLUME PART (basic/pm_demo.asm.ts, bakes 3 solids) — not just the golden fixture. Splits out of the old #948 tail; LEFT there = the P2 ParamsCard table editor + a TF parts_map builder (#974)." },
    { id: 974, bundle: 'B', lane: 1, start: 1.06, weeks: 0.12, priority: 'medium', status: 'done', title: "TF parts_map builder (SHIPPED — graph-to-tf.ts now has a parts_map case: expands the list<record> producer into N transformed instances, mirroring executeTfRecipe). Was: graph-to-tf.ts has NO parts_map case, so a parts_map node falls through to the default branch → UNSUPPORTED ('type parts_map has no TF mapping') and the native-only TF tab reports no builder for it. Add a lowerNode case that expands the list<record> producer into N transformed part instances (union / stack / place per the node's op), mirroring the Manifold executeTfRecipe path, so a pm_demo-style part builds NATIVELY in TF. Extends the parts_map node-card (#973) + the r_weld_extrude / composite TF builders (#942)." },
      { id: 975, bundle: 'E', lane: 4, start: 1.06, weeks: 0.10, priority: 'high',   status: 'done', title: "W-G · WSON→graph translator + embedded GRAPH tab (#42e, rungs 1–4). A well is drawn ONLY through the CAD graph mechanism — src/lib/wells/wson-to-graph.ts turns every structural section (oh / cementing / ch) into a bw_* Call placed down-hole by an Mv, and a deviated well adds ONE SplineNode sampled from its survey plus ONE multi-input WarpNode over every element (originZ: 0, so each element bends at its TRUE arc-length station rather than restarting at the spline origin). /wells gains a third viewMode mounting the real GraphEditorPane on that graph via seedGraph — no volume write; the pane's own Save creates w_<slug> under wells/. Output is a REAL composition Graph, not a wells-private format, so the part id is the seam where API + graph editor + well canvas meet (#65). Bake-verified: SAMPLE-1 → 9 parts, z 0..1070 (its TD), outer-r 13.0. Two bugs the bake caught that reading the code did not: bw_casing.top is declared but IGNORED (body hardcodes mv(...,[0,0,0])), so placement lives on Mv nodes; and CementInterval.od is the INNER diameter (the casing being cemented) — the outer is the bit size at the interval midpoint, mirroring WellSchematic3D. SHIPPED 2026-07-09." },
      ...
      { id: 977, bundle: 'B', lane: 1, start: 1.06, weeks: 0.06, priority: 'high',   status: 'done', title: "3D-bake section cap restored — buildSourceParts was DROPPING the cut face. Reported as '3D bake cutaway looks like a clip, TF looks like a proper CSG'. It was never a sectionCut bug: measured, sectionCut(solid,{az:180}) is a real capped CSG subtraction (genus 0, volume exactly half the uncut solid, positive sign, 2 tris on the cut plane). The cap was dropped at RENDER classification — the wedge is untagged, so its new cross-section faces carry Manifold's auto originalID (0xe), outside the part-hash band, and buildSourceParts discarded every triangle whose id it did not recognise (full cap=2, parts cap=0). The merged full/cutVC path and TF never drop by source id, which is exactly why those two showed a capped cut while the per-part 3D bake read hollow. Unknown ids now bucket into the body. Regression test src/lib/cad/sectioncut-render.test.ts. SHIPPED 2026-07-09." },
      { id: 978, bundle: 'B', lane: 1, start: 1.06, weeks: 0.05, priority: 'high',   status: 'done', title: "Part colour precedence — a part's own colour now beats the colour inherited from its body's deps (#86). bw_casing rendered GREEN and its swatch appeared dead: it declares colorOuter #460101, but its dep g_shaft declares #2ecd23, and the effective-appearance inheritance handed the dep's colour up. effectiveDepColor was already correct; the real gap was in analyzeParts (composite) and analyzeAssembly (.asm), where each instance resolved as instanceColors[name] ?? depColors[call] with no meta.colorOuter fallback in between. Precedence is now, per side: instanceColors → the part's OWN explicit colour → inherited dep colour → deterministic palette. Inheritance still carries up when the intermediate part sets nothing (that was the feature worth keeping). Affects every bw_* built on g_shaft. 4 regression tests. SHIPPED 2026-07-09." },
      { id: 979, bundle: 'B', lane: 1, start: 1.07, weeks: 0.05, priority: 'high',   status: 'done', title: "No silent client→server bake fallback — a client-bake failure is now an ERROR. PrimitiveDualCanvas used to catch ANY client-bake failure and quietly retry on /api/primitives/preview. Consequences, all observed in one session: a WASM trap in the browser's Manifold worker surfaced as a mystery SERVER 400 ('emval_methodCallers[caller] is not a function'), a string only preview/+server.ts can emit, so the bug looked server-side when it was in the client; the fallback request then ran the same geometry on the server, took the same trap, and corrupted the server's Manifold singleton so EVERY later bake 400'd with 'table index is out of bounds' or garbage arg types while prod baked the identical payload fine (the browser was poisoning the dev server through the fallback); and it made the ⚡/☁ badge look one-way, since the badge reports the backend that ACTUALLY produced the mesh, so a client bake that always fell back always read '☁ server'. The !supported branch still routes to the server deliberately — a capability gap (BREP sources), not a masked failure. SHIPPED 2026-07-09." },
      { id: 980, bundle: 'B', lane: 1, start: 1.12, weeks: 0.15, priority: 'high',   status: 'open', title: "Cut + warp — one bug cluster, probably one root. CONTEXT: g_shaft's body is now r_revolve({… zSegments: 0 …}) → sectionCut({az:180}), and it is the leaf of every bw_* part, so the whole well element library is half-sectioned. That is INTENDED (the cutaway must be an ACTUAL capped CSG part, not a view-only clip); it merely EXPOSED three things. (1) EXPLORE, the likely common root: is adaptive meshing skipped when a CUT part is warped? Manifold densifies the revolve PROFILE via subdivideProfileAxial(…, _axialMaxZSpan, …); TF walks a revolve-TREE via densifyRevolveTree. Does either see THROUGH a sectionCut/cutaway to the revolve beneath? Instrument both (station count + leaf kind per node) on w1_oh_warp BEFORE proposing a fix. (2) TF: a cut part collapsed under warp (762 tris / 2,286 verts, a thin lens, stray triangle). NOT fixed — execute.ts is byte-unchanged; the original reading was taken on a wedged dev server, and it now measures 1,584 tris / 4,752 verts, χ=2, 0 strays. Keep open until the densifier demonstrably walks through a cutaway. (3) 3D BAKE: warp of a cut part leaves stray tris — sectionCut builds its wedge with a bare 2-z-level extrude while the revolve IS densified, so the cut faces bend as straight chords. Strays fell 237 → 27 once the cap stopped being dropped at render (#977), so the residual 27 are the real geometry defect. DIAGNOSED 2026-07-10, NOT yet fixed: densifying the WEDGE is not enough — the `subtract` ITSELF retriangulates the planar cut faces, blowing maxEdgeΔz from 1.48 back to 40. The refine must be applied to the CUT RESULT, after the boolean, not only to the wedge going in. CAVEAT extrude(zlen, nDiv, 0) hits the manifold-3d degenerate-slices bug (nDivisions>0 + twist===0 → 'Not manifold'); use extrude(zlen).refineToLength(maxZSpan) or a welded wedge. Rule 25: segmentation at BUILD time, never a post-bake rewrite. (4) NARROWED 2026-07-10 — the suspect was WRONG. Each well on its OWN fresh dev server: 11-vertical → 200 (tris 498), 05-esp-producer (DEVIATED, warps) → 200 tris 84,902, 09-hpht → 400 'finalize-failed :: null function or function signature mismatch'. So it is NOT warp x sectionCut (05 warps and bakes), NOT deviation, NOT the imported placeholder parts (A/B swap changes nothing). Only well 09 traps, inside finalizeManifold. All 26 of its parts are individually valid and the 26-part M.compose succeeds (48,242 tris) but at genus -188 — the trap is downstream of that degenerate union. The earlier 'every well traps' reading was the #981 POISON CASCADE, not data. (5) SEPARATE BUG, same area: M.compose of a well's parts DESTROYS it (well 11: 16 parts -> 498 tris, genus 0), and preview:308's `typeof manifold.getMesh` plus render-helpers:160's `manifold?.numTri` are property reads that FORCE the lazy `_parts` proxy to compose. Make both guards _parts-aware. That is the 498-tri stick the /wells 3D pane renders. Memory: session_handoff_2026-07-10, manifold_trap_poison." },
      { id: 981, bundle: 'B', lane: 1, start: 1.13, weeks: 0.10, priority: 'high',   status: 'done', title: "Manifold trap guard — one WASM trap must not poison the process. SHIPPED 2026-07-10 (f894f0d). `manifold-trap.ts` = pure isManifoldFatalTrap + describeManifoldError, with NO manifold-3d import (bake-client is main-thread, where COEP require-corp forbids initialising Manifold). SERVER: withManifoldTrapGuard wraps the POST of both /preview and /bake-preview — it re-inits the singleton on a trap and rethrows a readable 500. It must unwrap SvelteKit's HttpError, because the endpoints call error(400, '<wasm msg>') BEFORE the guard sees the throw, so e.message reads '400'. CLIENT: a trapped worker is terminated and the job retried once on a fresh one; planTrapRecovery is pure and returns 'cancel' when a newer job is already waiting (re-queueing would overwrite it and hang that caller). Non-traps (404, bad params, a real 'Not manifold' CSG rejection) must NOT reset. 22 tests, incl. an end-to-end poison→recover: nulling the singleton is an UNFAITHFUL simulation (it yields 'M.cylinder is not a function', correctly ignored) — poison by making Embind methods throw the real emval_methodCallers string. THE MISDIRECTION IS THE EXPENSIVE HALF: a poisoned module names an INNOCENT part in its error, so a 400 on EVERY part ⇒ a poisoned module, not bad data. THE POISONER IS FOUND + FIXED 2026-07-10: the guard was being BYPASSED. preview's finalize catch did `return json({…},{status:400})` for every failure — a RETURN is not a THROW, so withManifoldTrapGuard never ran, never re-inited the singleton, and the module stayed corrupted; one real trap (well 09) then made every LATER bake fail with 09's stale message naming an innocent part. Now `if (isManifoldFatalTrap(e)) throw e;` — the guard resets and answers 500; an EMPTY-solid rejection stays an ordinary 400. A/B on a fresh server: BEFORE 09→400 then 11→400 + 05→400 (both echoing 09's `axisPath:10`, which well 11 does not even have); AFTER 09→500, 11→200, 05→200. Classification pinned in manifold-trap.test.ts. Memory: manifold_trap_poison." },
      { id: 982, bundle: 'B', lane: 1, start: 1.18, weeks: 0.20, priority: 'medium', status: 'open', title: "#66 Per-part material popover + bind-a-variable without wiring. (a) Today overriding a part's appearance means dropping a ◑ MaterialNode on the canvas and DRAGGING a wire into the Call card's material-in socket (graph.materialBindings[callId] = matId). Give each Call NodeCard a ◑ chip opening MaterialEditorPopover anchored to the card, writing straight to graph.partAppearance[callId]. Most of the machinery exists — PartAppearance, the partAppearance map, resolveEffectiveAppearance's precedence (materialBindings → partAppearance → part default), and the popover itself. Keep the wire path for REUSE (one material, many parts); the popover is the one-off override. (b) THE BET: pick a param/expr inside the popover instead of dragging a wire. Needs PartAppearance's fields widened to the ArgValue union (literal | expr | param) that Call args already use, plus a ƒ-style picker and a 'create a param from this value' action. Wires then become a VIEW of bindings rather than the only way to make them — collectEdges already derives edges from args, so a popover-created binding draws its own wire. That is what makes a 40-element well authorable: bind every casing's colour to one p.casingColor without 40 drags. TRAP: the per-part 3D viewer tint is still the deferred half of #86, so a popover built today could look like it does nothing — exactly the bw_casing green bug's failure mode. Fix the tint alongside." },

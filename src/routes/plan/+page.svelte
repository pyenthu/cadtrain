<script lang="ts">
  ...
  const BUNDLES: Bundle[] = [
    ...
    { id: 'H', name: 'Constrained parametrization (superseded)', tint: '#7c3aed', desc: 'SUPERSEDED by the source.ts component pivot — see bundle K. Parametrization now happens via source.ts components (meta.profiles + props), not a JSON DesignSpace/Generator. Original aim: API/vendor KB tables → DesignSpace + Generator → derived params → Svelte-runes runtime; replace freeform Opus param picking with coordinates in a constrained choice space.' },
    { id: 'I', name: '4-level hierarchy + composite generators', tint: '#0ea5e9', desc: 'Primitive → Composition → Component → Assembly. /components sidebar tabs + per-tier registries + rules files (tubing, drill_pipe) that take a small input set and emit a complete AuthoredComponent. KB-anchored generation, parent/child specialization in library.ts, in-tab Threlte rendering for every tier.' },
  ...
  const tasks: Task[] = [
    ...
    { id: 408, bundle: 'I', lane: 8, start: 12.5, weeks: 0.5, priority: 'medium', status: 'open',   title: 'I.8 — Bottom-Sub + Ratch-Latch in-tab port: write buildUnifiedManifold(flatParams) wrappers in each tool dir; register as ComponentDef so they open in /components instead of bouncing to /archive/tools/*' },
    ...
    { id: 606, bundle: 'K', lane: 10, start: 20.8, weeks: 0.4, priority: 'high',   status: 'active', title: 'K.6 — Sidebar directory restructure: tests→Industrial folder + Completions family folders DONE; Basic folder for r_* primitives pending' },
    { id: 607, bundle: 'K', lane: 10, start: 21.2, weeks: 0.6, priority: 'high',   status: 'open',   title: 'K.7 — Visual CSG/BODMAS composition tree for the return expr (operands=instances, nodes=union/subtract/intersect); recognizer parses composition → editable tree → splice' },
    { id: 608, bundle: 'K', lane: 10, start: 21.8, weeks: 0.5, priority: 'medium', status: 'open',   title: 'K.8 — Inspector UX polish: exclusive accordion (one row open unless pinned 📌), 2-col param layout (less padding/narrower cols), primitive title inside canvas (Threlte HTML) + hover description, profile shape-icon w/ hover preview + popup w/ 2dp coordinates, prominent tooltips (black bg/white text)' },
  ];

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 613, bundle: 'K', lane: 10, start: 23.6, weeks: 0.5, priority: 'medium', status: 'deferred',   title: 'K.13 — Warp z-spline revisit (PARKED): give the warp path its OWN popup (open polyline anchored at origin, not the closed-profile ProfileEditor) + fix the suspected interpretation bug (Z-down anchor z0=min.z=top→s=0; planar-only frame; x-centered assumption).' },

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 1024, bundle: 'F', lane: 5, start: 1.28, weeks: 0.06, priority: 'low', status: 'open', title: "#44 · Verb-guide token cut — relevant-only toApiMd (only the verbs a prompt needs) + drop the cloud-path tool-schema redundancy in the build prompt. Postponed until the local-model loop (#1020) shows prompt size is the bottleneck." },

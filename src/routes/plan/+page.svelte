<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 948, bundle: 'B', lane: 1, start: 1.02, weeks: 0.15, priority: 'medium', status: 'open', title: "Warp-trajectory + data-driven-params tails (from the 2026-07-07 batch, #972). #36c: wire the warp originZ placement option through to the node (the tangent-extension shipped; originZ exists but is unwired). #38: the data-driven params GEP node-card UI + the P2 param editor (P1 ParamSchema union + P3 parts_map producer + pm_demo already shipped). Plans docs/plans/{warp-part-along-spline,complex-params-list-of-parts,parts-params}.md." },
  ];

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 643, bundle: 'K', lane: 10, start: 63.2, weeks: 0.3, priority: 'medium', status: 'todo',   title: 'K.43 — A Phase 2 (DEFERRED): true GPU InstancedMesh for the live mesh + GLB renderer. place(parts) today CONCATENATES mesh data (no GPU instancing); finalize/upload scales linearly with total triangles (~2s at 12 joints, 211ms at 3). True InstancedMesh = one geometry + N transform matrices = flat memory/upload. Only build when long strings (50+ joints) become felt pain.' },
    ...
    { id: 803, bundle: 'M', lane: 12, start: 59.1, weeks: 0.6, priority: 'medium', status: 'todo', title: 'M.3 — Operator UX. Click two segments → fillet/chamfer with a LIVE radius dial; add a spline through picked points; offset for wall thickness; each writes a graph op + live re-bakes. Turns the operators from graph-authored (M.1) into direct-manipulation tools.' },

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id:  8, bundle: 'A', lane: 0, start: 57.4, weeks: 0.3, priority: 'low',    status: 'done', title: 'Add new primitive types as drilling needs surface' },
    ...
    { id: 613, bundle: 'K', lane: 10, start: 64.2, weeks: 0.3, priority: 'medium', status: 'done',   title: 'K.13 — Warp z-spline revisit (PARKED): give the warp path its OWN popup (open polyline anchored at origin, not the closed-profile ProfileEditor) + fix the suspected interpretation bug (Z-down anchor z0=min.z=top→s=0; planar-only frame; x-centered assumption).' },
    ...
    { id: 701, bundle: 'L', lane: 11, start: 57.9, weeks: 0.2, priority: 'medium', status: 'done',   title: 'L.2 — Public parts category: add `public` to LIBRARY_CATEGORIES (resolvers iterate the tuple) + visibility:public on save. Ships without identity.' },
    ...
    { id: 917, bundle: 'O', lane: 14, start: 68.5, weeks: 0.5, priority: 'medium', status: 'done', title: "O.13 — UX fixes (shipped e09932e): sidebar empty-folder visibility (filter gated on !filter.trim()), z-slider step=\"any\" (was a value-dependent step re-snapping the thumb), node-drag rAF-coalesce (full-graph re-render per pointermove → trailing). Plan: docs/plans/ux-bug-triage.md." },
    { id: 905, bundle: 'O', lane: 14, start: 69.5, weeks: 1.5, priority: 'high', status: 'open', title: "O.1 — Tab-aware multi-shot RAG assistant: build the Anthropic edit loop (ge-assist.svelte.ts) on the shipped Phase-1 tool registry + pure dispatcher; inject route/active-tab/open-graph + populate selectedId so \"edit THIS node\" works. Two bugs to fix: add /api/rag/assist to VOLUME_PROXY_PATHS; split buildAssistSystem so volatile graph state sits after the cache_control breakpoint. Plan: docs/plans/ai-rag-system.md." },
    { id: 906, bundle: 'O', lane: 14, start: 71, weeks: 1, priority: 'medium', status: 'open', title: "O.2 — Local web-llm backend (no data leaves org): web-llm (MLC) + XGrammar constrained decoding, Qwen2.5-1.5B in a Web Worker, default-OFF, gated by a bench (>=90% tool / >=85% args); local TF-IDF few-shot DB in IndexedDB. Reference Functionary's prompt format but do NOT deploy it (server-side vLLM, not WebGPU). Plan: docs/research/web-llm-functionary.md." },
    { id: 907, bundle: 'O', lane: 14, start: 72, weeks: 2, priority: 'high', status: 'open', title: "O.3 — Modularize round 2: dead-code sweep (builder.ts/library.ts legacy chain is dead; 12 unused deps + 90 unused exports), the still-live deja-vu bake-cache dep-hash fix (bake-cache.ts:109), + the E+F sketch consolidation (the full-tab editor + Sketch node card share sketchExprPop → one unit). Plan: docs/plans/modularize-round2.md + graph-editor-pane.md." },
    { id: 908, bundle: 'O', lane: 14, start: 74, weeks: 0.5, priority: 'low', status: 'open', title: "O.4 — Right-nav restructure: group the RightPane vertical rail into VIEW (3D/SVG/GLB/BREP) + DATA (SRC/MD) with a pinned settings entry sharing scene.controlsOpen; sectioned settings panel. Plan: docs/plans/right-nav-menu.md." },
    { id: 909, bundle: 'O', lane: 14, start: 74.5, weeks: 1, priority: 'medium', status: 'open', title: "O.5 — /design svelte-flow architecture graph: rebuild /design (currently an orphan, zero inbound links) as an interactive @xyflow/svelte graph of routes/api/lib, auto-synced via import.meta.glob + a drift-guard test. Doubles as the low-risk svelteflow pilot (NO-GO for the main editor). Plans: docs/plans/design-route-svelteflow.md + docs/research/svelteflow-for-graph-editor.md." },
    { id: 910, bundle: 'O', lane: 14, start: 75.5, weeks: 2, priority: 'high', status: 'open', title: "O.6 — Sketch repeat + Repeat windowed editor: (1) add poly_repeat-style repeat to the sketch; (2) open the Repeat card in its own in-tab window (sketch-overlay pattern) with PARAMS + iterators on top and two tabs — Loop body + graphical modifiers (NodeTransform[]). Plan: docs/plans/repeat-and-sketch-repeat.md." },
    { id: 911, bundle: 'O', lane: 14, start: 77.5, weeks: 2.5, priority: 'large', status: 'open', title: "O.7 — Client-side execution: server stays the compiler (graph->script, IP boundary, dep-inlined script-hash cache — also kills the deja-vu bug); client executes in a Web Worker. Manifold-client first (low risk); OCCT-client via replicad (lazy worker, ~11MB WASM, fallback). PRESERVE the server Manifold+OCCT builder under /api/server-builder/ (git mv + shims, never deleted). Plan: docs/plans/client-side-execution.md." },
    { id: 912, bundle: 'O', lane: 14, start: 80, weeks: 0.5, priority: 'low', status: 'open', title: "O.8 — Repo cleanup: archive ~19 stale scripts + 6 stale test specs + rm tests/results (105MB) + git mv training_data (175MB) -> archive; keep FEM + forge (forge flagged experimental). Awaiting approval per item. Plan: docs/plans/repo-cleanup.md." },
    { id: 913, bundle: 'O', lane: 14, start: 80.5, weeks: 1, priority: 'medium', status: 'open', title: "O.9 — TXFMN transform card: one TxfmnNode {child, rot[3], offset[3]} replacing composed Mv+Rot, re-expanded at emit to byte-identical mv(rot(child)); compact ROT/MV table with per-row f-expr + param socket; hydrate-time migration of legacy mv/rot. Plan: docs/plans/transform-card.md." },
    { id: 914, bundle: 'O', lane: 14, start: 81.5, weeks: 1, priority: 'medium', status: 'open', title: "O.10 — Expressions tab: third tab beside PARAMS/PROPS; sparse graph.exprs[] + e.<name> namespace (keeps p.* positional bake), pure graph-exprs.ts topo-order + cycle detection, server emits a const e = {...} prelude. Conditionals (ternary/min/max/clamp). Plan: docs/plans/expressions-tab.md." },
    { id: 915, bundle: 'O', lane: 14, start: 82.5, weeks: 0.5, priority: 'low', status: 'open', title: "O.11 — Normals + BREP.io: expose creaseAngle (re-bake, cache-keyed) + the smoothShade toggle (mesh normals are ALREADY crease-aware via Manifold.calculateNormals(3,60) — not a missing-normals bug). BREP.io = no spike (OCCT already server-side). Plans: docs/research/normals-smoothing.md + brep-io.md." },
    { id: 916, bundle: 'O', lane: 14, start: 83, weeks: 0.5, priority: 'low', status: 'open', title: "O.12 — /research route: index + viewer for docs/research/*.md via the already-present marked dep + import.meta.glob (no need to revive the archived MarkdownView); landing-page link. Plan: docs/plans/research-route.md." },
  ];

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 914, bundle: 'B', lane: 1, start: 89.5, weeks: 1, priority: 'medium', status: 'open', title: "Expression DEFINITIONS + instances (per-part library, v3 2026-06-24). A reusable expr definition (graph.exprDefs[]) with four declared sections — PARAMS (input sockets, wired per instance) / CONSTS (fixed locals) / VARIABLES (internal intermediates) / OUTPUTS (output sockets) — edited in a tabs-or-accordions builder (no graph p.* shown). Instances are ExprNode{defId,bindings}; editing a def updates all instances; emit namespaces per-instance + topo over params→consts→vars→outputs. Σ rail popover = the Expressions menu (list + ✎ + ⦻ drop-instance + +). SHIPPED: v2 block (model/card/emit/autocomplete). NEXT: PR-1 model+emit+migration, PR-2 four-section editor, PR-3 Σ menu, PR-4 instance card + e2e. Plan: docs/plans/expression-builder.md (v3)." },

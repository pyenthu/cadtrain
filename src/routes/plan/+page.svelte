<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 931, bundle: 'B', lane: 1, start: 98.3, weeks: 1, priority: 'medium', status: 'active', title: "tf (TrueForm) experimental CLIENT-side engine tab (in flight 2026-07-02). @polydera/trueform@0.9.8 added (free noncommercial eval; prod license separate); a 'tf' tab below BREP, WASM lazy-loaded on first open. Start with from-scratch shapes then TrueForm booleans on welded operand meshes. NOTE: the s_tube cap eval showed TrueForm's default boolean does NOT beat Manifold — building it as an engine OPTION per user direction, not as the cap fix. IN PROGRESS — @polydera/trueform@0.9.8 dep added (29b56c9); tab build stalled 3× at worker init → main-thread-first retry. Client+server both TODO. Plan docs/plans/trueform-engine-tab.md." },
    ...
    { id: 936, bundle: 'B', lane: 1, start: 100.4, weeks: 1, priority: 'medium', status: 'open', title: "Warp a part along a spline — a bend/deform modifier node reusing warp-spline.ts (warpManifoldAlongSpline). Needs build-time refine, right-handed-frame fix (warp inverts volume on a left-handed frame), self-intersection guard, and 3D-path frames (RMF). Plan docs/plans/warp-part-along-spline.md." },
    { id: 938, bundle: 'B', lane: 1, start: 100.7, weeks: 0.5, priority: 'low', status: 'open', title: "Sweep self-intersection check — warn on self-intersecting sweep sections (genus/volume or 2D segment-cross) instead of silent degenerate caps. Memory todo_sweep_self_intersection_check." },
    { id: 939, bundle: 'B', lane: 1, start: 101, weeks: 2, priority: 'high', status: 'active', title: "MULTI-ENGINE matrix — Manifold (client✓/server✓) · BREP/replicad (server✓, client TODO) · TrueForm/tf (both TODO), each a right-pane tab with a client/server toggle. Vision from 2026-07-02. Sub-work: 931 (tf tab), 934 (BREP client), 935 (BREP quality)." },
  ];

<script lang="ts">
  ...
  const BUNDLES: Bundle[] = [
    ...
    { id: 'J', name: '/primitives polish + warp', tint: '#06b6d4', desc: 'Inspector/stage/canvas UX pass on the runes /primitives page: merge Params into Parts with per-part accordion, drag-scrub number inputs, stacked 4-col card layout, 3D-only canvas with in-stage Mesh/GLB toggle, GLB cutaway + vertex-colours + Z× compression + brighter material, geom-accumulator DSL, sinusoidal Z-warp experiment (TEMP, behind a master checkbox).' },
  ];
  ...
  const tasks: Task[] = [
    ...
    // ───── J. /primitives polish + warp (2026-05-15 session) ─────

<script lang="ts">
  ...
  const BUNDLES: Bundle[] = [
    ...
    { id: 'B', name: '/primitives', tint: '#84cc16', desc: 'Visual parametric CAD editor: graph editor, sidebar, sketcher, external API, generative authoring, client-side bake, and session roadmap items.' },
  ...
  const tasks: Task[] = [
    ...
    { id: 512, bundle: 'A', lane: 0, start: 65.4, weeks: 0.2, priority: 'high',   status: 'todo',   title: 'AI refine Level 2: post-generation validation in the refine endpoint (imports allowlist · denylist scan · undefined-instance detection · syntax check · optional live-bake · retry-once-with-errors-fed-back)' },
    ...
    { id: 904, bundle: 'B', lane: 1, start: 73.2, weeks: 0.6, priority: 'medium', status: 'open', title: 'V1.3 LLM manifest + MCP: /api/v1/manifest (evolve /api/manifest) + /sdk/llms.txt + tool-call JSON schemas per operation + an MCP server (adopt SVTC\'s) so an agent can discover + drive cadtrain. Worked examples (author from description → bake → fetch GLB/SVG). V1.4 (later): embeddable canvas (iframe-postMessage or published web-component).' },
    ...
    { id: 691, bundle: 'B', lane: 1, start: 77.2, weeks: 0.3, priority: 'medium', status: 'todo', title: 'Edges-not-warping fix (2026-06-16, diagnosed, not yet fixed). On INSTANCED stacks ONLY, warp is baked into the canonical body\'s LOCAL z but instance copies are only translated, so copies 2+ carry the wrong warp phase and their edges stay straight (non-instanced parts warp fine). Fix options: (a) disable instancing when warp is on (simple, slower stacks) or (b) pass each copy\'s GLOBAL z-offset into the warp callback (keeps instancing fast — preferred). builder.ts tryInstanceFinalize.' },
    { id: 905, bundle: 'B', lane: 1, start: 77.5, weeks: 1.5, priority: 'high', status: 'open', title: "Tab-aware multi-shot RAG assistant: build the Anthropic edit loop (ge-assist.svelte.ts) on the shipped Phase-1 tool registry + pure dispatcher; inject route/active-tab/open-graph + populate selectedId so \"edit THIS node\" works. Two bugs to fix: add /api/rag/assist to VOLUME_PROXY_PATHS; split buildAssistSystem so volatile graph state sits after the cache_control breakpoint. Plan: docs/plans/ai-rag-system.md." },
    { id: 906, bundle: 'B', lane: 1, start: 79, weeks: 1, priority: 'medium', status: 'open', title: "Local web-llm backend (no data leaves org): web-llm (MLC) + XGrammar constrained decoding, Qwen2.5-1.5B in a Web Worker, default-OFF, gated by a bench (>=90% tool / >=85% args); local TF-IDF few-shot DB in IndexedDB. Reference Functionary's prompt format but do NOT deploy it (server-side vLLM, not WebGPU). Plan: docs/research/web-llm-functionary.md." },
    { id: 907, bundle: 'B', lane: 1, start: 80, weeks: 2, priority: 'high', status: 'open', title: "Modularize round 2: dead-code sweep (builder.ts/library.ts legacy chain is dead; 12 unused deps + 90 unused exports), the still-live deja-vu bake-cache dep-hash fix (bake-cache.ts:109), + the E+F sketch consolidation (the full-tab editor + Sketch node card share sketchExprPop → one unit). ✅ PHASE E DONE (Step 1 2026-06-16 639dc90+f86cdfd + Step 2 2026-06-23): SketchState class (sketch-state.svelte.ts, 710 lines) + SketchNodeCard.svelte (800d0e7) + SketchEditorPane.svelte (08ad8c4) — both take the per-pane sketch instance; the coord ƒ-popover stays in the GEP shell (the Phase-E-revert fix). GEP 8029→7376, browser-verified in /primitives (multi-instance) + /graph-editor. ✅ PHASE F DONE 2026-06-23 (aab47c1): NodeCard.svelte (2015 lines) extracted; GEP→6191. ✅ DEJA-VU BAKE FIX DONE 2026-06-23 (8edfb05, plan R1): depSourcesHash folds resolved meta.uses dep bodies into the cache key (collectDepSources/hashDepSources in primitive-loader, wired into api/primitives/preview, tested). LEFT: GEP still 6191 so ≤1500 needs one more feature carve — R6a Polygon/PolyRepeat editor out of GEP (Sketch-style) → R6b header + $state audit; plus GEP-independent R2 knip prune, R3 /design link-or-archive, R4 primitives Sidebar/TabStrip, R7 builder.ts/library.ts legacy retire, R8 vocab _tabs, R9 ProfileFnEditor split, R10 warp-subdivide retire + WASM-health. Plan: docs/plans/modularize-round2.md (§6 table) + graph-editor-pane.md." },
    ...
    { id: 909, bundle: 'B', lane: 1, start: 82.5, weeks: 1, priority: 'medium', status: 'open', title: "/design svelte-flow architecture graph: rebuild /design (currently an orphan, zero inbound links) as an interactive @xyflow/svelte graph of routes/api/lib, auto-synced via import.meta.glob + a drift-guard test. Doubles as the low-risk svelteflow pilot (NO-GO for the main editor). Plans: docs/plans/design-route-svelteflow.md + docs/research/svelteflow-for-graph-editor.md." },
    ...
    { id: 912, bundle: 'B', lane: 1, start: 88, weeks: 0.5, priority: 'low', status: 'open', title: "Repo cleanup: archive ~19 stale scripts + 6 stale test specs + rm tests/results (105MB) + git mv training_data (175MB) -> archive; keep FEM + forge (forge flagged experimental). Awaiting approval per item. Plan: docs/plans/repo-cleanup.md." },
    ...
    { id: 914, bundle: 'B', lane: 1, start: 89.5, weeks: 1, priority: 'medium', status: 'open', title: "Expressions tab: third tab beside PARAMS/PROPS; sparse graph.exprs[] + e.<name> namespace (keeps p.* positional bake), pure graph-exprs.ts topo-order + cycle detection, server emits a const e = {...} prelude. Conditionals (ternary/min/max/clamp). Plan: docs/plans/expressions-tab.md." },
    ...
    { id: 916, bundle: 'B', lane: 1, start: 91, weeks: 0.5, priority: 'low', status: 'open', title: "/research route: index + viewer for docs/research/*.md via the already-present marked dep + import.meta.glob (no need to revive the archived MarkdownView); landing-page link. Plan: docs/plans/research-route.md." },
  ...
  const codeById = $derived.by(() => {
    ...
    for (const bundleId of ['A', 'B', 'C'] as const) {

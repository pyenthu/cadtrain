<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 994, bundle: 'B', lane: 1, start: 1.30, weeks: 0.05, priority: 'medium', status: 'open', title: "Graph-editor LAYOUT changes must NOT trigger a rebake (user 2026-07-12). Moving/repositioning a node is pure visual layout — geometry is unchanged, so re-firing the bake is wasted work + jank. Key the bake trigger on the STRUCTURAL/semantic graph (node kinds, params, wires), not node x/y positions; persist layout without touching the bake hash. Related memory graph_editor_drag_bake_perf (never reassign graph per frame; bringToFront on pointerup; keep canvas mounted). Files: controller.svelte.ts bake orchestration + the graph-change effect. Verify with a REAL mouse drag — the bake badge must not flip." },

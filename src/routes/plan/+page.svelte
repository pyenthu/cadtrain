<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 1012, bundle: 'B', lane: 1, start: 1.84, weeks: 0.10, priority: 'medium', status: 'done', title: "DONE (44a9121 + 8b20647 + a485203) · WGPU render tab — a raw-WebGPU sibling of B·SVG that GPU-rasterizes the OCCT TRUE-boundary. /api/brep/svg gained format:'polylines' (edges + fill silhouette triangles + crease-aware fillShades + bbox; absent-format SVG byte-identical). WebGpuView: line-list outline pipeline + a triangle-fill pipeline sharing one fit uniform → clear-WHITE → steel fill → teal outline on top (aspect-fit + Y-flip). Dependency-free (raw WebGPU, all any-typed, degrades to a text panel with no device). +8 tests. GPU draw = human eyeball. Off the critical path." },
    { id: 1013, bundle: 'B', lane: 1, start: 1.85, weeks: 0.08, priority: 'high', status: 'done', title: "DONE (849c829 + bcad6a1) · BREP per-part / nested colours — bw_packer rendered all-brown because .add/.union fused the two OCCT solids (combineBool 'fuse') and carryTag kept only the BASE tag, losing the seal colour (a fused OCCT solid holds one tag; MF's mesh relation carries per-triangle source ids). Fix: a GROUP representation mirroring MF's lazy _parts — additiveCombine keeps a DIFFERENTLY-tagged operand separate as a group (same-tag/untagged still fuse → no-LUT path byte-identical), groups fold PER-ELEMENT through subtract/intersect/mv/rot/sectionCut/warpSpline; __tagNest implemented for real (partNestId). body.add(seal) now yields 2 colour regions (brown+blue); all 15 parity pins unchanged. +3 tests." },
    { id: 1014, bundle: 'B', lane: 1, start: 1.86, weeks: 0.01, priority: 'low', status: 'done', title: "DONE (ccc006a) · /primitives part-tab strip grows to fit its horizontal scrollbar — with enough tabs open, overflow-x:auto drew the scrollbar INSIDE the box, clipping the tab bottoms. Measure scrollWidth>clientWidth (raf + ResizeObserver) and reserve scrollbar room (padding-bottom + thin bar) ONLY while overflowing, so the active tab's margin-bottom:-1px merge into the pane is preserved when it isn't." },
    ...
    { id: 995, bundle: 'B', lane: 1, start: 1.35, weeks: 0.30, priority: 'medium', status: 'done', title: "DONE (6869649) · Modularized src/lib/graph/ — 39 loose root files → 13 concern subfolders (composition/expr/primitive/sketch/spline/port/part/editor/csg/survey/warp/wire/profile) alongside nodes/+stdlib/+tests/. Pure git mv + import rewrite (incl. ~180 relative external importers path-resolved), 3 landmines handled (engine-hash glob, folder-tree.ts 39 paths, graph/CLAUDE.md map), per-subfolder CLAUDE.md. Build green; 1852 pass / 1 skip / 1 pre-existing brep-svg failure (baseline-identical). tests/ consolidation (nodes/stdlib tests → graph/tests) left as a follow-up. Precedes #16." },
    ...
    { id: 948, bundle: 'B', lane: 1, start: 1.02, weeks: 0.12, priority: 'medium', status: 'done', title: "DONE · Warp-trajectory originZ + #38 P2. #36c: the warp node now exposes + applies originZ (warp.ts emits it + checkArg-validates it; multi-input warps default originZ 0). #38 P2: record|list params are authored/edited via the parts_table/parts_map list<record> cards (#973 node-card + pm_demo). Plans: warp-part-along-spline / complex-params-list-of-parts / parts-params." },
      ...
      { id: 75, bundle: 'B', lane: 1, start: 1.86, weeks: 0.06, priority: 'medium', status: 'done', title: "DONE (23ff9ed) · Auto-layout FORCE params — a force-organic layout pass (forceLayoutGraph in composition/composition-layout.ts: FR attractive spring on wired edges + charge repulsion; tension/repulsion/iterations/idealLength params; 6 tests) + tension + repulsion SLIDERS in the CanvasMenu ⚙ layout menu (persisted to localStorage; same ↶ undo path). ↑tension pulls wired cards closer, ↑repulsion spreads them. Confirmed good in-browser by the user." },
  ];

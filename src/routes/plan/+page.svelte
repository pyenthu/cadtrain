<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 907, bundle: 'O', lane: 14, start: 80, weeks: 2, priority: 'high', status: 'open', title: "O.3 — Modularize round 2: dead-code sweep (builder.ts/library.ts legacy chain is dead; 12 unused deps + 90 unused exports), the still-live deja-vu bake-cache dep-hash fix (bake-cache.ts:109), + the E+F sketch consolidation (the full-tab editor + Sketch node card share sketchExprPop → one unit). ✅ PHASE E DONE (Step 1 2026-06-16 639dc90+f86cdfd + Step 2 2026-06-23): SketchState class (sketch-state.svelte.ts, 710 lines) + SketchNodeCard.svelte (800d0e7) + SketchEditorPane.svelte (08ad8c4) — both take the per-pane sketch instance; the coord ƒ-popover stays in the GEP shell (the Phase-E-revert fix). GEP 8029→7376, browser-verified in /primitives (multi-instance) + /graph-editor. LEFT: Phase F (NodeCard per-node arms + polyExprPop), dead-code sweep, deja-vu bake fix. Plan: docs/plans/modularize-round2.md + graph-editor-pane.md." },

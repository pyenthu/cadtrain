<script lang="ts">
  ...
  const BUNDLES: Bundle[] = [
    ...
    { id: 'F', name: 'App harness', tint: '#7c3aed', desc: 'The strategic push — cadtrain as an ENGINE + a build-via-AI harness for sub-apps. A sub-app is a self-contained .app (component tree) an AI assembles by calling verbs (the verb registry = the API + the boundary). SHIPPED: the /app_design STUDIO (visual tree editor · ⚙ props · ✨ AI chat CLI/API/Phi · Variables/Data/Events/Style panes · props→runes-store auto-promotion · version-per-prompt snapshots · chat "needs work" → non-conformance) · server-render (/app/[id] · /app/local/[token]) · the LEARNING corpus on the volume (ai/app-rag builds.jsonl + goldens plan/design/ewell + component-cards) · the verb/route API tab in /design · three example apps. SHIPPED 2026-08 (#49/#50): a 34-kind component kit (+ stat/statgrid/chart/datatable/cad3d) · a RAG dictionary + atomic prompt library · the local-model eval loop RUN — in-browser Qwen2.5-1.5B lifted ~25%→44.9% vs the Claude reference (plan 100 · design 90 · ewell 69). NEXT: comprehensive cad3d + the component dispatch-engine / ADK direction (DEFERRED — cad3d-component.md), persistent-data + data-files tabs (DLIS/LAS), File-component slot picker, BREP client-side bake tab. Design: docs/architecture/app-harness.md; plans: app-server-render.md · app-build-eval.md · app-data-files.md · cad3d-component.md.' },
  ...
  const tasks: Task[] = [
    ...
    { id: 946, bundle: 'B', lane: 1, start: 1.05, weeks: 0.20, priority: 'low', status: 'open', title: "TF_WASM tab — C++→WASM TrueForm builders for IP concealment. NOTE the honest framing: the WORKER (off-main-thread) is what makes TF fast, NOT the language — WASM here buys source concealment, not speed. Plan docs/plans/tf-wasm-tab.md." },
      ...
      { id: 1011, bundle: 'E', lane: 4, start: 1.82, weeks: 0.20, priority: 'high', status: 'open', title: "#77 Wells = native GRAPH docs · WSON ≈ the composition-graph JSON (user 2026-07-28 DIRECTION). A well IS a graph document (well-part Calls + Mv + warp), authored/stored/edited through the SAME graph mechanism + engines — not a separate WSON schema + runtime translator; the new WSON converges to ≈ the graph JSON we already emit. FIRST STEP: basic .wson that mimics the current graph JSON (reproduce w2_completion_vert / w_multi_string). PARTS: recreate the completion/well elements as parametric VOLUME parts under well_parts/, referenced by id DIRECTLY — NO SVTC tool_comp parser (drop registry.ts/COMPLETION_PART_MAP); the #42j 'unmapped codes' problem dissolves. wsonToGraph degenerates to a one-time import of samples/*.wson. DECIDED (2026-07-28): a well is a PURE GRAPH DOC (.asm.ts); the well shape lives in typed list<record> params (casings/openholes/completions/survey via parts_table/parts_stack/parts_map + the #38 ParamSchema union, shipped) — NOT a 2nd WSON format; the 'thin WSON' IS the graph doc's param values. First artifact PROVEN (5129b7b): w_well_native — the 3 parts_table nodes lifted to list<record> params (cements/openholes/casings) + parts_map producers; bakes BYTE-IDENTICAL to w2_completion_vert (11538v/3846t/z300), so the param surface is faithful. Deliverable local (no volume write); tooling scratch/issue-77-param-well/. NEXT: save to the volume + build the params GUI. Runtime render already graph-native (#42h CORE DONE: /wells 3D bakes wsonToGraph via GraphEditorPane; LEFT = TF selector + rail→3D toggles + delete the dead WellSchematic3D/WellScene/manifoldCut shell). Consolidates the retired unify-format/sample-ladder/load-from-volume/editing WSON TODOs. Memory wells_empty_3d_wiring_gap." },
    { id: 1020, bundle: 'F', lane: 5, start: 0.98, weeks: 0.18, priority: 'high', status: 'done', title: "#49 · Local-model replication — LOOP RUN. Claude CLI *incremental* reference locked (plan 100 · design 90 · ewell 69). In-browser Qwen2.5-1.5B lifted ~25% → 44.9% avg via: an incremental-discipline fix (kills front-load drift, Claude 70→100), an un-copyable few-shot example (kills verbatim example-copy, plan 31→51), RAG grounding wired to the browser (/api/app/ground), and an 8192 context window. Per-rung eval diagnostic (/app_design/eval + scripts/eval-app-build.ts --incremental); scorer score-app.ts. REMAINING: meta 0% (docType/title) on design/ewell · design's dropped multi-var seeds · promote atoms as goldens · retrieval quality · try Qwen2.5-3B. Plan: docs/plans/app-build-eval.md. ⚠ 2026-08-16: the BROWSER figures above stand (that path took the 8192 context fix on 08-02), but every HEADLESS number quoted anywhere near them is void — see #1029 for the four harness bugs. The REMAINING list is measured against a broken harness and is partly stale: 'meta 0% on design/ewell' is NOT what the first trustworthy baseline shows (design meta 50%, ewell 50%, and partsdash went 50→100 for free on the context fix alone). Re-verify each item against runs/baseline-5app.md before working it — and note the headline score hides the shape: pre/post-fix runs both scored ~38% on partsdash while producing COMPLETELY different builds, so read facets, not the total. 'retrieval quality' is now the concrete #1028." },
    { id: 1021, bundle: 'F', lane: 5, start: 0.98, weeks: 0.12, priority: 'high', status: 'active', title: "Persistent data + data files — a .app runs from a local DATA file plus colocated sibling files (DLIS/LAS) in the same directory. SVTC model (studied): ONE showDirectoryPicker handle persisted in IndexedDB + file references {name,path,type} in the doc (NOT handles) + resolve-on-load re-link against the reopened directory tree (no per-file re-pick). Two studio tabs: Persistent data (the main source + open-workspace) · Data files (siblings, add via tree/picker, link-status, re-link-all). Plan: docs/plans/app-data-files.md." },
    { id: 1022, bundle: 'F', lane: 5, start: 1.14, weeks: 0.10, priority: 'medium', status: 'open', title: "#24 · File component + slot picker in the VISUAL editor — wire the File component + a data-file slot picker into VisualEditor wiring so an author binds a component's source to a §0.5 file slot (pairs with #1021 persistent data)." },
    { id: 1023, bundle: 'F', lane: 5, start: 1.20, weeks: 0.05, priority: 'medium', status: 'open', title: "Per-button popover wiring — PanelNode today attaches only the FIRST popover child of a parent, so a rail of N iconbuttons all share ONE popover (ewell). Pair each iconbutton with its OWN popover (by id / adjacency) so each toolbar button opens its own panel." },
    { id: 1024, bundle: 'F', lane: 5, start: 1.28, weeks: 0.10, priority: 'medium', status: 'open', title: "#44 · Verb-guide token cut — relevant-only toApiMd (only the verbs a prompt needs) + drop the cloud-path tool-schema redundancy in the build prompt. RE-PRIORITISED 2026-08-16 high→medium: prompt SPACE is not the constraint. The k-ceiling sweep showed even k=20 costs only ~1275 tok of ~3459 spare, and the 5-app baseline showed more examples HURT the strong apps — so freeing budget buys nothing on its own. What it does buy is attention/quality, which is real but unquantified. NOTE the related finding that IS load-bearing: prompt REDUNDANCY costs far more than length — removing the style props' 34 duplicate copies from the component cards (~500 tok) moved plan 20.0→48.0 in a same-corpus comparison (9ba7900). Cut duplication first, breadth second. Earlier note (2026-08-10): the precondition ('postponed until #1020 shows prompt size is the bottleneck') is MET — and not marginally. The headless Ollama runner sent no num_ctx, so a ~4.7K-token systemPrompt overflowed Ollama's 4096 default and was truncated SILENTLY; grounding sits near the top, so it was the first thing cut, and RAG did literally nothing headless until ab5f4ae. Measured budget (partsdash, chart prompt, 17513 chars ≈ 4733 tok): toApiMd(VERBS) 8635 ch / ~2334 tok = 49.3% of the prompt · component-knowledge cards 6084 ch / ~1644 tok (ALREADY prompt-filtered — leave alone) · hand-written instructions 2794 ch / ~755 tok. 30 verbs are documented in full but only the 14 'gui' ones are dispatchable — the other 16 exist purely to be REFERENCED in bindings, so cutting them to signatures frees ~4366 ch / ~1180 tok. NOTE the constraint has MOVED: at num_ctx 8192 the prompt fits with ~3459 tok spare, so this now buys small-model attention/quality, not capacity — the direct lever on capacity is k (rankGolden's 3-golden cap, a CODE constant), sweepable via the new APP_RAG_K (7ac2148). Grounding is worth +4.3pp (partsdash) / +14.5pp (opsdash), so prompt budget spent on examples has a measured payoff." },
    { id: 1028, bundle: 'F', lane: 5, start: 1.34, weeks: 0.12, priority: 'high', status: 'open', title: "#49b · Atomic goldens for plan / design / ewell — THE top lever, from the first trustworthy 5-app baseline (2026-08-16, runs/baseline-5app.md). Those three have NO retrievable goldens at all: every atomic golden is atom-dash-*, and the full-app .md keys they used to rely on were unseeded in a8311b4 (correctly — a whole app is the wrong exemplar SHAPE for one atomic prompt). It shows: plan at k=3 built a single [container], panelKinds 0.0%, score 8.0%, against opsdash's 72.7%. Raising k to 12 only helps them because it drags in WRONG-DOMAIN dashboard atoms that at least teach shape (+23.3pp on plan) while costing opsdash -15.0pp — the mirror image of the 2026-08-02 cross-app contamination finding. So k is app-dependent, not a constant to tune; goldens are the fix. Slice each app's golden into per-component atoms the way gen-atomic-dashboard-goldens.ts does for the dashboards, docType-tagged + md-keyed, into scripts/eval-fixtures/golden/atomic/. Then RE-RUN the k grid — do not assume the current numbers hold." },
  { id: 1029, bundle: 'F', lane: 5, start: 1.20, weeks: 0.10, priority: 'high', status: 'done', title: "#49a · Eval-harness reliability — SHIPPED 2026-08-11..16. FOUR stacked bugs, each invisible and each hiding the next, which between them invalidated every headless number the project had: (1) num_ctx unset → Ollama's 4096 default truncated a ~4.7K-token systemPrompt SILENTLY, and grounding sits near the top, so RAG never reached the model at all (ab5f4ae); (2) Bun caps fetch at 300s and an AbortSignal does NOT override it — needs timeout:false (961f9cb); (3) num_predict unset + llama-server --context-shift = a repetition loop that NEVER terminates: design had literally never produced a score, 2 of its 13 rungs still hit the 2048 cap (b8eb52e); (4) the ad-hoc runner piped evals through grep, DISCARDING stack traces, so (3) presented as 'no score' twice — replaced by a resumable per-cell runner (961f9cb) + OLLAMA_TRACE. Consequence: the 44.9%/46.1% averages, the lexical-vs-vector hill-climb, and both factory trials are void; the 20-of-21 NA hill-climb rows were (3), not 'flakiness'. LESSON: this path is temperature-0 DETERMINISTIC, so any score change is provably a cause, not noise — and every prompt-affecting change needs a before/after, because there is no noise band to hide in." },
  { id: 1025, bundle: 'F', lane: 5, start: 1.36, weeks: 0.15, priority: 'low', status: 'open', title: "#33 · JS-code escape hatch (FUTURE) — a friendly per-component code editor, server-compiled + variable-aware, for logic the declarative verbs/computed can't express. Reuses the graph/expr safe interpreter." },
    { id: 1026, bundle: 'F', lane: 5, start: 0.98, weeks: 0.14, priority: 'high', status: 'done', title: "#50 · Component kit + RAG dictionary — SHIPPED (34 kinds). New dashboard/Excel/3D components: stat · statgrid · chart (SSR-SVG bar/line/area/pie/donut) · datatable (sort/search/paging/totals) · cad3d (interactive 3D CAD viewer — a dataMode:'client' island; new computeMode field = 'server', bakes via /api/app/cad-bake, engine/source never ship). RAG dictionary SSOT-generated (docs/rag/app-builder-dictionary.md) + atomic prompt library (docs/rag/sample-prompts.md) so the AI/local model composes them." },
    { id: 1027, bundle: 'F', lane: 5, start: 1.51, weeks: 0.25, priority: 'medium', status: 'deferred', title: "Dispatch engine + comprehensive cad3d (ADK direction) — DEFERRED (user 2026-08-02; spec docs/plans/cad3d-component.md). A component needing bespoke server work ships a <Name>.server.ts (never enters the client bundle) → a generated SERVER_ACTIONS registry → ONE generic route /api/app/component/[kind]/[action] (server never grows a route per component), fronted by a verb (AI SSOT). Comprehensive cad3d: render-mode selector (MF mesh · BREP · SVG) · popover graph editor · volume part-picker. Long game: cadtrain graph/engine componentized as an ADK embedded in other apps — cad3d is instance #1." },
  ];

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 624, bundle: 'K', lane: 10, start: 29.8, weeks: 1.2, priority: 'high',   status: 'open',   title: 'K.24 — File-based P0: taxonomy + migration + server. <id>.<kind>.ts files (prvl/prex/prim/asm — mid-ext=type); volume-only; migrate existing source.ts→.prim.ts and profile.json+source.ts→.prvl.ts/.prex.ts; server routes bake/resolve by mid-ext + bake cache keyed on content+params+dep-hashes, busted on save. See docs/plans/file-based-architecture.md.' },

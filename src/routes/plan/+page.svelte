<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 990, bundle: 'B', lane: 1, start: 1.42, weeks: 0.20, priority: 'medium', status: 'open', title: "IDEA · a BREP_SVG tab — shade the SVG from BREP boundary SURFACES (exact faces/edges/analytic normals), not a projected triangle soup: clean true-edge outlines + smooth per-region fills. Server-side OCCT (HLRBRep hidden-line removal → 2D outline curves; per-face normal → Lambert fill). Complements #985. Research OCCT HLR first." },
    ...
    { id: 932, bundle: 'B', lane: 1, start: 0.89, weeks: 0.25, priority: 'medium', status: 'open', title: "Parametric geometry slots — expose a part's internal geometry (path spline / section expr/profile) as an OVERRIDABLE typed param with a default; the caller wires a compatible value (e.g. s_tube shares one path into two sweeps, rad still parametric). Typed-param-with-default, NOT inheritance. Part-level generalization of wire-into-spline (#26). Plan: parametric-geometry-slots.md." },

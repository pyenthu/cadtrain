<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 662, bundle: 'K', lane: 10, start: 46.5, weeks: 0.8, priority: 'high',   status: 'done',   title: 'K.62 — Assembly composition model overhaul: lists-are-groups + per-row CSG chain + nested sub-lists (2026-05-29..31, four commits 72786dc / 5fbb96f / b8c0de5 / 7a8e2fd). User-driven model shift away from K.56-D\'s single-op-per-row pill (▢add ▣subtract ◫intersect ▤place) which was unintuitive once the "list IS group" mental model was on the table. NEW MODEL — what the user sees in source: `return [A, B];` is the canonical compose (sandbox auto-place wraps any returned Array in a recursive Manifold.compose); CSG ops are authored as a LEFT-to-RIGHT chain per row (`const A = A_raw.subtract(B).intersect(C);`); groups are inner arrays (`[A, [C, B, D.intersect(E)]]` ⇒ nested place()). PHASE E.1 (72786dc) — sandbox wrapper installed in buildPrimitiveGeom auto-places Array returns recursively; Instance.ops {op, arg}[] data field; two-pass emitter (PASS1 `const NAME_raw = <placement>` + downstream tail/head refs lock onto `_raw` so a .subtract doesn\'t crop the column\'s stacking math; PASS2 ops chains); Instance.hidden field excludes operand-only rows from the return list; backward-compat migration on parse lifts legacy `op:\'subtract\'|\'intersect\'` to the previous row\'s ops chain + marks operands hidden. PHASE E.2 (5fbb96f) — per-row ⊕⊖∩ mini-toolbar: click an op button opens a FloatingPanel typeahead listing sibling row names; pick one and it appends to Instance.ops; existing ops render as `[∆ C ×]` chips with × to remove; auto-hide on append + auto-un-hide when the last reference is removed; 👁 toggle on hidden rows to also show them in the scene as an override; the single-op `<select>` and the currentOpForInstance / setInstanceOp helpers are GONE. PHASE E.3 data (b8c0de5) — Instance.children?: Instance[] for nested groups; recursive emit walks groups depth-first (children first, then `const NAME = [child names];`); sequential mate cursor is LOCAL to each group (entering resets, exiting locks onto the last visible child\'s tail); ops chains + overlay anchors cross groups freely (every binding in the same function scope); parser STRIPS `children: [...]` BEFORE other field scans so a regex field-getter doesn\'t reach into nested rows and pull a child\'s src/args up to the group; walkInstances / flatInstances public iterators. PHASE E.3 UI (7a8e2fd) — `+ Group (sub-list)` button at the top of the add-part popup creates an empty {children: []} at top level; group marker rows render as `🗂 G [B] [C] ✕` chips in the Sequential subtab; ✕ deletes the group AND promotes children back to top level so nothing\'s lost; filteredAsmParts excludes any row inside a group\'s children list. DEFERRED to E.3.1 (NEXT — next commit) — (a) RECURSIVE NESTED RENDERING: render each group\'s children as full expandable accordions inside the group\'s body (their own ops bar / mode pickers / drag handles / color swatches), not the flat chip list; needs a Svelte 5 snippet `{#snippet renderRow(inst, depth)}` that recursively renders + the per-row machinery extracted into a reusable inner snippet; (b) DRAG-INTO-GROUP: drag a top-level row onto a group\'s body area to reparent (mutate meta.instances tree: remove from top, append to group.children); (c) DRAG-OUT-OF-GROUP: drag a child out to the spaces between top-level rows to promote; same tree mutation in reverse; (d) GROUP-LEVEL ops on the group ITSELF (`const G_raw = [B, C]; const G = G_raw.subtract(F);`) since groups are first-class composables too; (e) DRAG-TO-SIBLING-GROUP move across groups. All E.3.1 items are pure UI on top of the data-layer foundation that E.3 (b8c0de5) shipped, so they\'re independent of further runtime work. Bake regression baseline confirmed at each phase: legacy flat assemblies still produce 297712-byte GLB through the new wrapper (no source rewrite needed); new-shape assemblies bake equivalently or better. Tracks the K.56 → K.62 evolution; supersedes the per-row op pill model from K.56-D entirely (the op-pill markup is removed from PrimitiveView).' },

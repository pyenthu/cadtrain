<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 907, bundle: 'B', lane: 1, start: 80, weeks: 2, priority: 'high', status: 'open', title: "Modularize round 2 — shell cleanup + dead-code sweep. GraphEditorPane is 6191 lines; reaching a ≤~1500-line composing shell needs one more feature carve. REMAINING: R6a — extract the Polygon/PolyRepeat editor out of GraphEditorPane (poly-preview overlay + vertex drag + SVG-insert + the poly-expr-pop family, ~700 lines, same pattern as the Sketch carve), then R6b — module-map header + residual $state audit + record the honest post-carve line count. GEP-independent tracks (isolated-worktree subagents): R2 knip dead-code prune (builder.ts/library.ts legacy chain + 12 unused deps + 90 unused exports + mime.ts/temp-file.ts), R3 /design link from landing (being rebuilt as the interactive xyflow graph), R4 primitives → Sidebar + TabStrip, R7 builder.ts → builder-legacy + render-helpers then retire library.ts, R8 vocab → _tabs/*, R9 ProfileFnEditor preview ↔ round-trip split, R10 warp-subdivide retire + WASM-health banner. Plan: docs/plans/modularize-round2.md (§6 sequencing table)." },
    ...
    { id: 909, bundle: 'B', lane: 1, start: 82.5, weeks: 1, priority: 'medium', status: 'done', title: "/design svelte-flow architecture graph: rebuild /design as an interactive @xyflow/svelte graph of routes/api/lib/store. ✅ DONE 2026-06-23: ArchGraph + ArchNode (source/target Handles) + curated architecture.ts (29 nodes / 26 edges), @xyflow/svelte@1.6.1, legend + minimap + controls; page made scrollable (height:100% + overflow-y). Proves the svelteflow pilot. FOLLOW-UPS: C4 Container-view template (group/parent boxes); auto-sync nodes via import.meta.glob + drift-guard test; dagre auto-layout. Plans: docs/plans/design-route-svelteflow.md." },
    ...
    { id: 914, bundle: 'B', lane: 1, start: 89.5, weeks: 1, priority: 'medium', status: 'open', title: "Expression BLOCK node (v2 design 2026-06-23): a first-class canvas node with DECLARED input sockets (wired in by connectors) + MULTIPLE named outputs (each a formula + output socket). Formulas use only local input/output names — graph wiring supplies values (no global e.* namespace). Reuses the merged mathjs engine (graph-exprs.ts/expr-schema.ts) as the formula validator + the expr/*.svelte popover as the block editor (declare inputs + name=formula rows + autocomplete over locals+functions, ternary conditionals). Wires via the existing WireState/socket system; emit substitutes wired inputs + local topo over outputs. SHIPPED: PR-1/PR-2 engine (model/topo/validation) + PR-3 popover UI (text-ref v1). NEXT: ExprNode model+emit → node card with sockets → block editor. Plan: docs/plans/expression-builder.md (v0 section)." },

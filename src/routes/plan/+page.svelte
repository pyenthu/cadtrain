<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 905, bundle: 'A', lane: 0, start: 77.5, weeks: 1.5, priority: 'high', status: 'done', title: "Tab-aware multi-shot RAG assistant — SHIPPED: the Anthropic edit loop (ge-assist.ts + ge-assist.svelte.ts) on the Phase-1 tool registry + pure dispatcher; /api/rag/assist added to VOLUME_PROXY_PATHS; buildAssistSystem split so volatile graph state sits after the cache_control breakpoint; AiMenu edit mode mounted. RESIDUAL (route/active-tab context + selectedId on node-click so \"edit THIS node\" works) moved to the AI MASTER PLAN P1. Superseded by docs/plans/ai-master-plan.md (north star; was ai-rag-system.md)." },
    ...
    { id: 923, bundle: 'B', lane: 1, start: 94, weeks: 3, priority: 'medium', status: 'active', title: "Expression-as-builder — structured + list expression outputs that unify the 3 repeats (idea 2026-06-26, from the spiral exploration). **IMPERATIVE LOOP BUILDER + CodeMirror SHIPPED 2026-06-28 (pushed 4808497):** list<point> outputs are now BUILT/EDITED as a visual imperative accumulator program (poly = []; for i = 0 to N { rx = …; rz = …; poly.append([rx, rz]) }; return poly) — src/lib/cad/expr-imperative.ts (parse/serialize/compile→a JS for-loop; bracket-depth statement split so wrapped/temp-var bodies parse as one; validateImperative w/ real error reasons) + ExprImperativeBlocks.svelte (◇ poly accumulator · structured for-headers w/ count dropdown · ONE resizable autocomplete body text field per loop · top '+ add ▾' = variable/expression/loop · top-level vars) + ExprCodeEditor.svelte (CodeMirror 6, lazy-loaded — DSL highlight + bracket match + autocomplete + line-wrap). Emit branches on isImperative→compileImperative (bakes IDENTICAL: spiral 2888 / functional 8628 verts). UX: output tabs promoted into the popover TITLE row; Enter=newline (data-enter-newline opts the body out of GraphEditorPane's window Enter→re-bake) + a ✓ update tick per loop (dirty-glow + commit-on-blur Save flush); functional map/concat auto-CONVERTS to imperative on open; ExprLoopBlocks.svelte deleted; g_spiral_expr_sketch re-saved on the volume in the readable R/a/rx/rz form. Plan: docs/plans/loop-builder.md. LEFT (the actual unify payoff): wire list<op>→sketch + list<transform>→repeat to retire the 3 repeat types; lacing; the visual '+' compose operator. Today expr OUTPUTS are scalars. Enhance so an output carries a scalar | object | LIST, with a map(range(N), i => …) loop inside, and the typed output SOCKET wires the structured value into any consumer: list of [r,z] → polygon points (= poly_repeat), list of ops → sketch ops (= sketch_repeat), list of transforms → place/repeat (= part-repeat). ONE generic generator subsumes all three repeat node types — the spiral collapses to ONE expression with a map. The engine (mathjs) already supports arrays/objects/map/range, so the work is MODEL (a `shape` field on outputs + non-scalar validation), a loop UI affordance, TYPED output sockets + wiring rules (list→list-slot, object→object-slot), and emit-splice into the consumer's array slot — NOT a new evaluator. Substrate for a 'builder' app (wire points/ops/transforms/parts between typed expr outputs) and data-driven parts (a list output sourced from a param table / CSV / the RAG layer → heterogeneous repeats fall out free). Research prior art FIRST (deep-research pass): Grasshopper data-trees + list-lacing (the reference design + its tree-matching pain), Dynamo list-lacing rules, Blender Geometry Nodes/Sverchok fields, OpenSCAD list comprehensions, CadQuery eachpoint, Houdini copy-to-points. Open Qs: type-system depth (scalar|object|list vs richer records), loop-in-formula vs structured for-each block, perf of a 720-pt mathjs map vs the emitted Array.from. Pairs with the repeat-as-sweep / loft-between-copies idea (clean continuous swept solids, see g_spiral_repeat.md). Plan: docs/plans/expression-list-builder.md (now grounded by a 104-agent deep-research pass: start FLAT list<element>, longest-repeat-last lacing, socket-shape typing, NO data trees, avoid Grasshopper graft/flatten). DATA-MODEL step IN FLIGHT 2026-06-26 (subagent): shape+elem on outputs, map()-list validation, list<point>→polygon points, spiral-as-one-expression bake proof. Three spiral demonstrators (g_spiral / g_spiral_sketch / g_spiral_repeat in basic/spirals/) motivate it." },
    { id: 924, bundle: 'B', lane: 1, start: 94.5, weeks: 1.5, priority: 'medium', status: 'open', title: "Repeat-as-sweep — loft a continuous welded skin BETWEEN consecutive repeat copies (geometry; idea 2026-06-26, from the spiral exploration). The part-level repeat today place()s N DISCRETE copies (g_spiral_repeat = ~26k verts of overlapping box-posts, bumpy). A 'sweep' mode lofts ONE welded skin between copy i and i+1 along the per-iteration transforms → a swept solid: springs, threads, helical ramps, clean spiral ribbons (~g_spiral's 8.6k verts). Welded-mesh path (manifold-mesh.ts gridPatch/capFan/weldAndBuild — like r_weld_extrude but along a path, not straight z). PROTOTYPE IN FLIGHT 2026-06-26 (subagent): a sweepAlongPath/loftStations helper + spiral bake proof + design writeup. Open problems: per-station frame torsion/flips, variable-radius spacing, self-intersection at tight inner turns, end caps, volume-sign check. Pairs with #11 (expression list outputs). See docs/parts/g_spiral_repeat.md." },
    { id: 925, bundle: 'B', lane: 1, start: 95, weeks: 0.6, priority: 'medium', status: 'done', title: "r_sweep — general section-along-3D-path engine (extrude a fixed 2D cross-section along an arbitrary 3D path → ONE welded watertight tube via sweepAlongPath→loftStations→weldAndBuild, no CSG). SHIPPED on main 2026-06-30: engine + declared data params (fixes the empty-params object→positional bake bug), the graph-editable sweep_demo part, and a 'sweep' item in the solids add-node menu (dropSolid seeds a round section + L-bend path). r_surface got the same empty-params fix (branch). Distinct from #924 (repeat-as-sweep). Generalization to a varying per-station section (→ subsumes r_loft) is future (TODO #23)." },
    { id: 926, bundle: 'B', lane: 1, start: 95.6, weeks: 0.8, priority: 'medium', status: 'active', title: "Typed expression outputs — structural inference + dynamic wiring (the design spine, refined live 2026-06-29..07-01). Phase A SHIPPED on main: struct-type.ts inferStructure/structLabel + an 'auto' output shape + live inferred-type badge; array literals like [[x,y,z],…] are accepted (killed 'unsupported syntax: ArrayNode') and inferred as list<point2/3/N> at any arity; scalar/list paths byte-identical. Model: type inferred from structure by DEFAULT (users never declare), explicit annotation optional (gradual typing, a contract checked vs inference), named types optional. Phase B (typed/colored sockets + plain-language wire-checking — refuses 2D-into-path) + Phase D (Type-Definer: first-class list types, the 'add a field' name-required bug, field-type dropdown) DONE on agent branches, pending review/merge. LEFT: C explicit annotation · E consumers (r_sweep.path / r_surface_grid) + ObjectNode emit + record→array adapter. Plan: docs/plans/typed-expression-outputs.md. Subsumes the wire-it half of #923/#13." },
    { id: 927, bundle: 'B', lane: 1, start: 96.4, weeks: 0.7, priority: 'medium', status: 'done', title: "Spline-editor PATH card — SHIPPED on main 2026-07-01. A three.js/Threlte popover (Canvas + OrbitControls + interactivity, no three/examples/jsm) with draggable control-point spheres + a Catmull-Rom curve; OUTPUT = N equally-spaced (arc-length) points via a PURE-JS resampler (spline-resample.ts — no THREE at bake) → list<point3> → r_sweep.path → smooth curved tube. Also: visible output wire (extended the wire renderer for spline source nodes), ground PROJECTION of the curve (grey footprint, not a rendered shadow), click-the-curve-to-insert a point mid-run (clickable tube), camera-LOCKED drag (capture-phase pointerdown raycast + stopImmediatePropagation — no orbit/rescale while dragging; focus target frozen mid-drag), scrollable XYZ hand-edit table (3-dp), 📌 pin (non-modal — edit while using the bake canvas), minimal curved-glyph node card, 3-dp coord rounding. Fixed en route: the effect read+write freeze loop, and the client/server bake toggle not re-baking (badge looked stuck on server). TODO #15. Ref: three.js webgl_geometry_spline_editor. Pairs with #926 (typed outputs)." },
    { id: 928, bundle: 'B', lane: 1, start: 97, weeks: 0.8, priority: 'high', status: 'done', title: "r_sweep QUALITY — twist + smooth normals + non-degenerate caps (SHIPPED 2026-07-02). (1) Rotation-minimizing frame (double-reflection, sweepFrames in manifold-mesh.ts) kills section twist on 3D/near-Z paths — the old fixed-up side=t×up0 rolled + degenerated near the Z axis. (2) Crease-aware smooth normals (render-helpers.ts creaseAwareCornerNormals, position-welded adjacency) fix the STRAIGHT-prism BLOCKY render — calculateNormals returns valid-but-PER-FACET normals for a perfect prism, so the recompute now ALWAYS runs under a 60k-vert ceiling (not just on all-zero). (3) fanCap3D ear-clips (Newell plane) so non-convex sections cap cleanly (was a convex-only vertex-0 fan → tangled caps). KERNEL_VERSION nrm3→nrm4→cap1. Commits ffb2b10/acff393/1c5dc65/2614c84. Memory r_sweep_normals_and_twist." },
    { id: 929, bundle: 'B', lane: 1, start: 97.8, weeks: 0.5, priority: 'medium', status: 'done', title: "BREP r_sweep — exact-kernel sweep under OCCT (SHIPPED 2026-07-02, 5daf667). Mapped r_sweep onto replicad genericSweep (OCCT MakePipeShell+MakeSolid) in brep-occt.ts (polyline spine — a BSpline spine hangs MakePipeShell on near-cusp paths). WHY: the exact B-rep boolean gives a CLEAN annular cap (0 degenerate/0 sliver) on the curved concentric hollow subtract where Manifold's MESH boolean corrupts the tilted coplanar caps (~137 tris). ~40-100x slower; display-mesh T-junctions (OCCT tessellation, not holes) need a global weld before an auto-fallback." },
    { id: 930, bundle: 'B', lane: 1, start: 98.3, weeks: 1, priority: 'high', status: 'done', title: "Annular / CSG-2D cross-section sweep — the DURABLE hollow-tube fix (MERGED 2026-07-02, b048817). sweepAnnular in manifold-mesh.ts builds a hollow tube as ONE welded mesh (0 degenerate, genus 1) — the durable defect-2 fix. Sweep an annular section (outer + inner hole loop, or a CrossSection with holes) ONCE → one welded mesh with a holes-aware cap, NO 3D CSG → no tilted-coincident-cap slivers (defect 2), engine-agnostic + faster. Phase 2 = an xSection SKETCH CARD composing multiple 2D sketches via 2D CSG into one section. Plan docs/plans/annular-csg2d-section-sweep.md." },
    { id: 931, bundle: 'B', lane: 1, start: 98.3, weeks: 1, priority: 'medium', status: 'done', title: "tf (TrueForm) experimental CLIENT-side engine tab — SHIPPED 2026-07-02 (merge 3f4989e). @polydera/trueform@0.9.8 (free noncommercial eval; prod license separate); a 'tf' right-pane tab below BREP, WASM lazy-loaded on first open, renders a from-scratch box on the main thread (proof of init after 3 worker-init stalls → main-thread-first). REQUIRED cross-origin isolation (COOP/COEP) added app-wide in hooks.server.ts + a vite dev middleware to unlock SharedArrayBuffer for TrueForm's pthreads (merge 8f205bd; verified non-regressing, prod-facing invariant). NOTE: the s_tube cap eval showed TrueForm's default boolean does NOT beat Manifold — it's an engine OPTION, not the cap fix. LEFT: per-part geometry (currently only the demo box) + client/server toggle. Plan docs/plans/trueform-engine-tab.md." },
    { id: 932, bundle: 'B', lane: 1, start: 99.3, weeks: 1.5, priority: 'medium', status: 'open', title: "Parametric geometry slots — spline/expr/profile as an OVERRIDABLE typed param (design 2026-07-02). A part exposes its internal geometry (path spline, section expr/profile) as a typed param with a DEFAULT; the caller overrides by passing/wiring a compatible value (e.g. s_tube wires one shared path into two sweep instances while keeping rad parametrized). Typed-param-with-default, NOT class inheritance. The PART-LEVEL generalization of wire-into-spline (#26). Plan docs/plans/parametric-geometry-slots.md." },
    { id: 933, bundle: 'B', lane: 1, start: 99.5, weeks: 0.5, priority: 'high', status: 'done', title: "Scene freeze fix (effect_update_depth_exceeded, SHIPPED 2026-07-02). ROOT: /primitives mounts ALL tab panes at once + scene (scene-state.svelte.ts) is a MODULE SINGLETON → N panes on differently-sized parts ping-pong the shared xScale/zScale/partCenter → freeze (same as the never-root-caused task 920). Fix: autoScaleOwner={active && rightTab===...} — only the active tab's visible canvas writes scene (commits 951a98a/40cd3ce/12e5c3d). Rule in src/lib/shared/CLAUDE.md. Also fixed sweep_tube_demo d_circle /12→/num_pts self-intersecting-section." },
    { id: 934, bundle: 'B', lane: 1, start: 99.8, weeks: 1, priority: 'high', status: 'open', title: "BREP client-side (OCCT/replicad WASM in-browser) + a client/server toggle like Manifold's ⚡/☁. replicad=opencascade.js runs in the browser; move the BREP bake into a lazy-WASM Web Worker; keep the /api/brep/preview server path as the toggle's other half. Plan docs/plans/brep-client-side.md." },
    { id: 935, bundle: 'B', lane: 1, start: 100.1, weeks: 1, priority: 'medium', status: 'open', title: "BREP display-mesh quality: (a) color parts (respect colorOuter/Inner + #86 subpart colors — BREP renders monochrome today); (b) smooth normals (crease-aware on the OCCT tessellation, like the Manifold fix); (c) the ~1.8e9 bogus tri/vert count + non-watertight display mesh — brep-occt.ts:160-162 Array.from over raw emscripten heap VIEWS inflates counts; fix = slice to OCCT's reported node/tri counts, then position-weld + Manifold.merge for watertightness (per the 2026-07-02 cleanup spike)." },
    { id: 936, bundle: 'B', lane: 1, start: 100.4, weeks: 1, priority: 'medium', status: 'open', title: "Warp a part along a spline — a bend/deform modifier node reusing warp-spline.ts (warpManifoldAlongSpline). GEOMETRY CORE SHIPPED 2026-07-02 (merge c12873f): 3D RMF path frames + right-handed-frame fix (was inverting volume on a left-handed frame) + build-time validity warning, all in warp-spline.ts. LEFT (deferred pending the GEP modularization reorg #940): the user-facing warp NODE + editor card — a large GraphEditorPane subsystem (items 1 & 5 of the plan). Plan docs/plans/warp-part-along-spline.md." },
    { id: 938, bundle: 'B', lane: 1, start: 100.7, weeks: 0.5, priority: 'low', status: 'done', title: "Sweep self-intersection check — SHIPPED 2026-07-02 (merge 71e55a5). Build-time 2D segment-cross check (sectionSelfIntersects in manifold-mesh.ts) warns on non-simple sweep sections instead of silently producing degenerate caps; geometry unchanged. Memory todo_sweep_self_intersection_check." },
    { id: 939, bundle: 'B', lane: 1, start: 101, weeks: 2, priority: 'high', status: 'active', title: "MULTI-ENGINE matrix — Manifold (client✓/server✓) · BREP/replicad (server✓, client TODO) · TrueForm/tf (tab✓ demo-box only, per-part + toggle TODO), each a right-pane tab with a client/server toggle. Vision from 2026-07-02. Sub-work: 931 (tf tab ✓ SHIPPED — from-scratch box + COOP/COEP cross-origin isolation), 934 (BREP client), 935 (BREP quality)." },
    { id: 940, bundle: 'B', lane: 1, start: 101.5, weeks: 2, priority: 'high', status: 'active', title: "GraphEditorPane modularization reorg (round 3 / R6-continuation) — THE CURRENT FOCUS (2026-07-02). GEP has drifted back to ~5,328 lines and must be reduced toward a ~1,500-line composing shell (the honest 2026-06-25 target from #907 was ~2,500–3,000; this reorg re-pushes past it). An audit was just completed; the next cut is the layout-actions extraction, following the R6/R2–R10 sequence in docs/plans/graph-editor-pane.md + docs/plans/modularize-round2.md. Unblocks the deferred warp NODE + editor card (#936, items 1 & 5). Prior modularization: #665 (round 1), #907 (round 2, GEP 9455→5070)." },
  ];

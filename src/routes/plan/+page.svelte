<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 512, bundle: 'J', lane: 9, start: 67.9, weeks: 0.4, priority: 'high',   status: 'todo',   title: 'J.12 — AI refine Level 2: post-generation validation in the refine endpoint (imports allowlist · denylist scan · undefined-instance detection · syntax check · optional live-bake · retry-once-with-errors-fed-back)' },
    { id: 513, bundle: 'J', lane: 9, start: 68.3, weeks: 0.2, priority: 'high',   status: 'todo',   title: 'J.13 — AI refine Level 3: live-bake gate on the inspector Accept button — status pill ("✓ Builds" green / "✗ Bake failed: <msg>" red); Accept disabled on failure. Uses the existing /api/components/bake-preview endpoint, no backend changes' },
    { id: 514, bundle: 'J', lane: 9, start: 68.5, weeks: 0.3, priority: 'medium', status: 'todo',   title: 'J.14 — AI refine Level 4: assembly-aware prompt — when refining a composition, glob docs/assemblies/README.md + matching <assembly>.md into the system prompt. Today nothing in src/ reads docs/assemblies/ so the AI re-invents known recipes every refine' },
    ...
    { id: 613, bundle: 'K', lane: 10, start: 73.3, weeks: 0.5, priority: 'medium', status: 'deferred',   title: 'K.13 — Warp z-spline revisit (PARKED): give the warp path its OWN popup (open polyline anchored at origin, not the closed-profile ProfileEditor) + fix the suspected interpretation bug (Z-down anchor z0=min.z=top→s=0; planar-only frame; x-centered assumption).' },
    ...
    { id: 700, bundle: 'L', lane: 11, start: 68.8, weeks: 1.0, priority: 'high',   status: 'open',   title: 'L.1 — OAuth identity port from SVTC: Google OAuth + signed-session → event.locals.userId via sequence() in hooks (existing AUTH_TOKEN/proxy/rate-limit unchanged). Plan ready: docs/plans/oauth-identity.md. Blocked on user-provisioned Google OAuth creds.' },
    { id: 701, bundle: 'L', lane: 11, start: 69.8, weeks: 0.3, priority: 'medium', status: 'open',   title: 'L.2 — Public parts category: add `public` to LIBRARY_CATEGORIES (resolvers iterate the tuple) + visibility:public on save. Ships without identity.' },
    { id: 702, bundle: 'L', lane: 11, start: 70.1, weeks: 1.0, priority: 'high',   status: 'open',   title: 'L.3 — Private per-user parts under components/<userId>/ (REQUIRES L.1): user-scoped resolvers + owner enforcement; close R2 (/api/volume path guard), R3 (private out of proxy), R4 (list-cache by userId), R5 (id-collision scoped).' },
  ];

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 914, bundle: 'B', lane: 1, start: 89.5, weeks: 1, priority: 'medium', status: 'open', title: "Expression builder (popover, expandable to full — NOT a persistent tab): author calculated expressions with a SRC pane + a strictly-controlled visual block builder (mathjs typed AST ↔ custom Svelte blocks; controlled p.*/e.* inputs + named outputs; per-block AST-walk validation). Backed by sparse graph.exprs[] + e.<name> namespace (keeps p.* positional bake), pure graph-exprs.ts topo-order + cycle detect, emit const e_<name> prelude. One or multiple outputs. Plan: docs/plans/expression-builder.md." },

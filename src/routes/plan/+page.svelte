<script lang="ts">
  ...
  const BUNDLES: Bundle[] = [
    ...
    { id: 'O', name: 'Session roadmap (2026-06-16 planning batch)', tint: '#14b8a6', desc: '16 plan/research docs produced by the parallel planning subprocesses (docs/plans + docs/research): tab-aware multi-shot RAG assistant + local web-llm, modularize round 2 (+ deja-vu bake fix), right-nav restructure, /design svelteflow graph, sketch-repeat + repeat windowed editor, client-side execution (server=compiler / client=executor) preserving the server builder, repo cleanup, TXFMN transform card, expressions tab, normals creaseAngle/smoothShade, /research route, and the shipped UX fixes.' },
  ];

<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 1002, bundle: 'B', lane: 1, start: 1.72, weeks: 0.10, priority: 'high', status: 'done', title: "DONE (f086b4d) · parts_stack node kind (#38e) — a HETEROGENEOUS auto-stacked 'completion string' card: each ROW is a DIFFERENT element part (per-row src), mated END-TO-END in order via the sandbox stack([...]) helper → one geometry (vs parts_table's one-template, rows-render-separate). Backend: PartsStackNode/PartsStackRow{src,args?,material?} + PartsStackKind (emitExpr → stack([_ps_<id>_i]), inputRefs [] inline srcs, one output socket, grow-to-fit); emitPartsStackBlocks prelude (each row its own src); every row src → meta.uses; per-row material → meta.instanceColors; NOT a list producer (stack = one mated value); hydrate sanitize + mutate family (add/remove/duplicate/move rows, setRowSrc clears stale args, setRowArg, setRowMaterial); predecessorsOf leaf guard. 13 unit tests. User req 2026-07-22." },
    { id: 1003, bundle: 'B', lane: 1, start: 1.74, weeks: 0.10, priority: 'high', status: 'done', title: "DONE (51745d8) · parts_stack CARD UI + row reorder (#38e). PartsStackCard (decoupled row table: each row = an element part-type picker + its LENGTH inline; ▲▼ reorder = stack order; two-click delete-confirm; ＋ element) + StackRowPopover (the row's ⚙ — the element's OTHER meta.params, length excluded, + a material section reusing the material→colour override + '↳ matl' flag; portaled to body). NodeCard parts_stack arm (⊟ marker, one mated-string output) + GraphEditorPane 'completion string' menu item in container › + per-row src picker keyed (nodeId,rowIdx) + params-fetch effect over every row src. Verified in-browser: bw_hanger + bw_tubing mate end-to-end as one bake; ⚙ lists od/id/chamfer + material; commit → re-bake. FOLLOW-UP: card-level master material + a typed-create scaffold entry (deferred)." },
    { id: 1004, bundle: 'B', lane: 1, start: 1.71, weeks: 0.06, priority: 'medium', status: 'done', title: "DONE (760bafa) · parts_table card polish (#38d): (1) a CARD-LEVEL master material — a black|white 'material sphere' in the TITLE row (new tableMaterial node field) that skins the WHOLE table when set, applied as a BASE to every row via meta.instanceColors + the TF rowAppr merge; a per-row override still WINS field-by-field. (2) DELETE is now two-click (🗑 arms → ✓/✕; no native confirm() — it wedges the in-browser extension). (3) the delete button moved from the left cluster to a sticky RIGHT-WALL cluster just before the row's output socket, socket flush to the card edge. Verified in-browser on w2_multi_part_warp + 6 new unit tests." },
    { id: 1005, bundle: 'B', lane: 1, start: 1.70, weeks: 0.05, priority: 'low', status: 'done', title: "DONE (7e0869d + 7511a1a + 5923e22) · material/appearance batch. (1) Row material popover compacted to ONE uniform row (swatch · material · opacity · clear) + downhole non-metallic presets added (sandstone/limestone/shale/cement/mud alongside the metals). (2) Selecting a material now OVERRIDES the row colour with that material's own tint (steel→blue-grey, sandstone→tan, …) and FLAGS it with a '↳ matl' marker in the popover; picking a custom colour clears the flag. (3) Default scene lighting lifted (ambient 0.7→0.9, tone-map exposure 1.35→1.55 — MF_CLIENT read dim) — still SceneControls knobs. (4) cement preset reworked lighter·browner·greyer·rougher (#b7b2aa→#c8bfb0, roughness→1). The preset→material→render (roughness/metalness) path is wired for all three engines." },
    ...
    { id: 994, bundle: 'B', lane: 1, start: 1.30, weeks: 0.05, priority: 'medium', status: 'done', title: "DONE (8537450) · Graph-editor LAYOUT changes no longer trigger a rebake (user 2026-07-12). Moving/resizing a node is pure visual layout, but the auto-bake trigger keyed on emittedForRender.source — which embeds meta.graph (node x/y/w/h + view scales) — so every drag re-emitted a different string → a full rebake. Fix: a layout-independent bakeKey = emitGraph's `body` (geometry, topo-ordered so layout-invariant) + the appearance meta the canvas paints (instanceColors/partAppearance/colours/opacity/texture/material) + param defaults + uses; the auto-bake debounce + the 'stale' badge key on bakeKey, not the raw source. Canvas already reads bake.source (only updated on a real bake) so it doesn't re-bake on move either. Verified: browser network shows ZERO /compile after a node move; headless emit-layout-invariant test proves emitGraph.body is byte-identical across a layout change, differs on a real arg edit. Files: GraphEditorPane.svelte." },

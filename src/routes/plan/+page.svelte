<script lang="ts">
  ...
  const BUNDLES: Bundle[] = [
    ...
    { id: 'E', name: '/wells', tint: '#0ea5e9', desc: '3D-first well schematic (SVTC WsonApp-modeled). Shell shipped (sidebar + tabs + workspace header + wired 3D). Continuation: left toolbar rail, dual 3D/2D canvas (2D = SVG track), view/layer controls, editor/inspector dock, annotations + depth ruler. Plan: docs/plans/wells-interface.md.' },
  ];
  ...
  const tasks: Task[] = [
    ...
    { id: 953, bundle: 'E', lane: 4, start: 0.06, weeks: 0.14, priority: 'high',   status: 'open', title: 'W-D · Dual 3D/2D canvas — 2D SVG well-track view (3D already live). Port SVTC wsonRender.js → pure src/lib/wells/wson-2d.ts (depthToY shared with the 3D depth scale, radiusToX with diaScale; casing/tubing/oh/ch rects, cement hatch, perf arrows, buildDirPath deviation, leader labels) + WellSchematic2D.svelte rendering inline <svg>. Mount beside WellSchematic3D (toggle/split), same layer+scale state. SVG content/styling per user spec.' },
    { id: 950, bundle: 'E', lane: 4, start: 0.20, weeks: 0.08, priority: 'high',   status: 'open', title: 'W-A · View + layer control bar — surface WellSchematic3D existing props (layer toggles oh/ch/cement/tubing/completions/perfs · cutaway · directional · diaScale/zScale/DTX · cut azimuth · camera presets) as a SceneControls-style toolbar over the canvas. Drives BOTH 3D + 2D.' },
    { id: 951, bundle: 'E', lane: 4, start: 0.28, weeks: 0.14, priority: 'high',   status: 'open', title: 'W-B · Right editor/inspector dock — CompletionsEditor (strings table: oh/ch/cement/tubing/completions rows) + SurveyEditor (md/dev/az stations, live re-warp) + click-to-select inspector wired to the parametric registry (tool_comp → its ParamSpec dials). SVTC CompletionsEditor/SurveyEditor.' },
    { id: 952, bundle: 'E', lane: 4, start: 0.42, weeks: 0.10, priority: 'medium', status: 'open', title: 'W-C · On-diagram annotations + depth ruler — MD + DTX-remapped depth axis down the side; leader-line labels for components/perfs (3D billboards or a side-rail label column).' },
    { id: 955, bundle: 'E', lane: 4, start: 0.0,  weeks: 0.05, priority: 'high',   status: 'active', title: 'W-F · Workspace CACHE — persist + restore the /wells workspace across visits (no re-picking the folder). Last-opened FileSystemDirectoryHandle in IndexedDB (handles are structured-clone-able, NOT localStorage) + re-request permission via a "Reopen <folder>" affordance on load; open .wson tab list + active index in localStorage; hydrate files once permission is regranted. SSR-safe, degrades to empty workspace. (In progress — agent a5003e8e.)' },
    { id: 956, bundle: 'E', lane: 4, start: 0.05, weeks: 0.05, priority: 'high',   status: 'open', title: 'W-G · Workspace shell polish — (a) COLLAPSIBLE detail header (SVTC schematic style): well name + description, type (vertical/producer), TD, PBTD, and csg/comp/perf/svy counts, expand/collapse; (b) reduce the big canvas padding/margin (.wv-stage) so the 3D view fills the stage; (c) 2D SVG canvas on a WHITE background. Small inline tweaks to src/routes/wells/+page.svelte + WellViewPlaceholder — done after the W-F cache agent merges (same files).' },

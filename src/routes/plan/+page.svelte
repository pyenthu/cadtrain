<script lang="ts">
  ...
  const tasks: Task[] = [
    ...
    { id: 909, bundle: 'B', lane: 1, start: 82.5, weeks: 1, priority: 'medium', status: 'done', title: "/design svelte-flow architecture graph: rebuild /design as an interactive @xyflow/svelte graph of routes/api/lib/store. ✅ DONE 2026-06-23: ArchGraph + ArchNode (source/target Handles) + curated architecture.ts (29 nodes / 26 edges), @xyflow/svelte@1.6.1, legend + minimap + controls; page made scrollable (height:100% + overflow-y). Proves the svelteflow pilot. ENRICHED + COLLAPSIBLE 2026-06-25: C4 container view grown to 56 nodes / 65 edges (corrected stale /fem,/forge → archived; added /research + /api/brep) then rebuilt as a COLLAPSIBLE auto-layout TREE — system→containers→components, recursive LR layout, collapsed-by-default, useSvelteFlow().fitView re-frame on every toggle (SvelteFlowProvider in +page). FOLLOW-UPS (TODO #16): tune the expand re-center + spacing/styling toward the mfmesh.up.railway.app/architecture reference; legend overlaps the rightmost column at fit-zoom. Plans: docs/plans/design-route-svelteflow.md." },
    ...
    { id: 920, bundle: 'B', lane: 1, start: 91.5, weeks: 0.3, priority: 'high', status: 'open', title: "/primitives multi-tab restore FREEZE (regression introduced with the expr v3 merge, 2026-06-25). Restoring a /primitives session that reopens several persisted tabs at once hard-hangs the renderer (synchronous loop — no console error, screenshot times out). /graph-editor single-pane is fine; /primitives loads fine with NO tabs and works when you open one part. So it's a MULTI-PANE / specific-migrated-part interaction with the v3 mount (a WELLS part carrying migrated v1 graph.exprs → exprDefs is the prime suspect). Workaround: clear localStorage prim-open-tabs. NEXT: reopen the 7 parts (g_barrel/g_mandrel/g_shaft/g_cube/g_dp_joint/casing_sudo/casing_schematic) one at a time to find the offender, then fix the loop (likely an effect/derived cycle in the instance card or migration when N panes mount). HIGH priority — it's on the main working surface." },

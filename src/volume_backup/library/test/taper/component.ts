...
export const meta = {
  ...
  params: {
    odTop: { label: 'OD Top', min: 0.5, max: 6, step: 0.1, unit: 'in', default: 2.1 },
    odBottom: { label: 'OD Bottom', min: 0.5, max: 6, step: 0.1, unit: 'in', default: 3.0 },
    wall: { label: 'Wall', min: 0.05, max: 1, step: 0.05, unit: 'in', default: 0.35 },
...
export const geom = defineGeom(meta, (p, geom) => {
  ...
  const A = TubeGeom({ od: 2.875, wall: 0.375, length: 1, top: 0 });
  geom.add(A);
});

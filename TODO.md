...
- **#46 Graph → TF compiler = the "actual" button (the real one)** — a translator that compiles the composition graph/JSON → a sequence of TF instructions (booleans · `tfRevolveProfile` lathe · `tubeMesh` · transforms), shipped to the client and executed NATIVELY in the loaded TF kernel (server does only the lightweight compile — could even be client-side; one small call, no geometry server-side; all heavy work fast + local). Generalizes the hand-written `tf_examples/*.ts` into an automatic emitter (like composition-emit but targeting TF). Coverage: CSG→booleans ✓, revolve→lathe ✓, extrude/sweep→tubeMesh-ish, fillets→sharp approx, some nodes unmapped (documented). This is what the "actual" button should do; #43 (import the baked mesh) is a simpler stepping-stone/complement.

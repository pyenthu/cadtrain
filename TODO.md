...
- **✅ Shipped:** rung 1 (verbs→AI-SDK schema→dispatch) · rung 2 (harness renders `.app`) · dynamic loader (load/list/save at runtime + `scaffold.app`) · rung 3a (data verbs → REAL parts) · **rung 4a** (AI build primitive: prompt → gui verbs → `.app`, `/api/app/generate` + ChatPanel — **VERIFIED LIVE**: built a hello-world text panel) · **4a hardening** (sanitizeApp drops hallucinated verbs) · **rung 4a.2** (learning loop: `app-corpus.ts` captures every build + grounds future ones — lexical, local) · **ENG-gui** (iterative-editing verbs: removePanel/movePanel/setPanelProp/setAppMeta) · **rung 3b-data** (`bake` verb → real geometry stats via source→preview; Bake3dPanel shows verts/tris) · **ENG-engine** (`getSource` + `compile` verbs) · **rung 5** (provider abstraction `providers.ts`: cloud Anthropic default / local Ollama via `APP_BUILD_PROVIDER=local` — build-green; live local run needs Ollama = human verify) · **4b-MVP** (`VisualEditor`: palette + add/remove/reorder/rename panels via the gui verbs; ✎ Edit toggle in `/app/[id]`; drag-to-place = follow-up).
- **✅ Autonomous loop COMPLETE.** Remaining (need browser/user, not autonomous): 3b interactive 3D canvas (Threlte over the bake) + mutate `setParam`/`addRow` → re-bake · 4b drag-to-place (mine Svelte Visual Builder) · **PENG** per-engine RAG/LLM (user authors w/ Claude). Reference for hello-world→wells: this repo `wells.app`/`/wells` + SVTC + wellnew. **Browser follow-ups (need human verify):** 3b interactive 3D canvas (Threlte mount over the bake) + mutate `setParam`/`addRow` → re-bake. Then **PENG** per-engine RAG/LLM (user authors w/ Claude). Reference for hello-world→wells: this repo `wells.app`/`/wells` + SVTC + wellnew.

...
- **Browser follow-ups (need human verify, not autonomous):** 3b interactive 3D canvas (Threlte mount over the bake) + mutate `setParam`/`addRow` → re-bake · 4b drag-to-place (mine Svelte Visual Builder) · **PENG** per-engine RAG/LLM (user authors w/ Claude). Reference for hello-world→wells: this repo `wells.app`/`/wells` + SVTC + wellnew.

...
- **SHIPPED:** the STUDIO (`/app_design`) — visual tree editor · ⚙ props · ✨ AI chat (CLI·API·Phi) · Variables/Data/Events/Style panes · props→runes auto-promotion (store = source of truth) · version-per-prompt snapshots · light-dismiss popovers · server-render (`/app/[id]`·`/app/local/[token]`) · API tab in /design. The LEARNING corpus (`ai/app-rag/`): `builds.jsonl` + curated goldens + component-cards; `/api/app/{promote,learn,ground}`. **RAG dictionary** (SSOT-generated, **34 components · 30 verbs**, `docs/rag/app-builder-dictionary.md`) + **atomic prompt library** (`docs/rag/sample-prompts.md`). **Component kit:** dashboard + Excel + 3D — `stat`·`statgrid`·`chart`·`datatable`·`cad3d` (the last a server-compute client island; new `computeMode` field on `ComponentMeta`).
...
- **NEXT:** `sheet` (formula columns on `datatable`) · author **dashboard goldens** with the new kit → promote (teach the kit to the RAG/local model) · dashboard `<Name>Editor.svelte` + a cad3d param-binding UI · File component + slot picker (#24) · verb-guide token cut (#44) · JS-code escape hatch (#33, FUTURE).

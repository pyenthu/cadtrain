...
- **✅ Shipped:** rung 1 (verbs→AI-SDK schema→dispatch) · rung 2 (harness renders `.app`) · dynamic loader (load/list/save at runtime + `scaffold.app`) · rung 3a (data verbs → REAL parts) · rung 4a (AI build primitive: prompt → gui verbs → `.app`, `/api/app/generate` + ChatPanel; builds captured to `_builds.jsonl`).
- **▶ Next:** 3b bake→live-3D + mutate (setParam/addRow→re-bake) · **4a.2** progressive pipeline + RAG grounding/capture → iterate **hello-world → wells** (reference: ours/SVTC/wellnew) · **ENG** widen the verb API to the FULL engine surface (compile/preview/save/engine ops) · **PENG** per-engine RAG/LLM · **4b** native visual `.app` editor (Svelte Visual Builder patterns) · rung 5 local runtime + flywheel.

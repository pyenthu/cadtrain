...
17. **Components render two ways, picked per-entry by `renderMode` on the `/api/components/list` response:**
    ...
    **Cross-instance prop refs (`expandInstancePropRefs`)**: before transpile, the loader scans every `(let|const) X = call(args)` base declaration to build an `<INST>.<prop>` → raw arg-text map (helpers use positional `manifold-helpers-meta` props; components use object-literal keys from the imported component's `meta.params`). Then it replaces every `<INST>.<prop>` reference in the source with the matching literal. Lets the user write live cross-instance refs in the editor — e.g. `B = mv(B, [A.length, 0, 0])` — without `A.length` being undefined at runtime. The reference text stays on disk (so editing A's length cascades to B on the next preview/save).

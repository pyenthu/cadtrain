...
22. **FEM domain is encapsulated — `src/lib/cad/fem/` (engine) + `src/routes/fem/` (UI) + dedicated subagent for substantial changes (2026-06-01).** Finite-element analysis is its own domain alongside the CAD core. Layout:
    - **Engine** `src/lib/cad/fem/`: pure logic (closed-form stress today; pure-JS linear FEA / WASM tet meshing / CalculiX subset port in future stages). NO Svelte / DOM / Three deps here. The full multi-stage roadmap + library research lives at `src/lib/cad/fem/FEM.md`; per-directory contract at `src/lib/cad/fem/CLAUDE.md`.
    - **UI** `src/routes/fem/`: `+page.svelte` (index — lists tubular primitives), `[id]/+page.svelte` (per-primitive stress). Imports formulas from `$lib/cad/fem/*` only — UI never duplicates equations. Per-directory contract at `src/routes/fem/CLAUDE.md`.
